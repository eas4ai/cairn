...
findings:
  - resolved: pkg-lint reads .muse-plugin/plugin.json under PKG-008 but its declaration omits .muse-plugin/, so a change there leaves the lint's evidence current when it is not. Resolved: .muse-plugin/ is declared to pkg-lint beside the other two manifests (052912c)
