...
findings:
  - open: the README and the install skill do not tell a user who registered the hooks by hand, from the settings snippet, to remove those entries when the plugin is installed; both sets would run, printing two verdicts at session start and refusing a stop twice.
