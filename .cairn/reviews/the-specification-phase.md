...
findings:
  - resolved: tests/skills.test.mjs matches substrings that include the skills' line breaks, so a reflow of a paragraph that changes no word fails the test; it is checking wrapping, not content
  - resolved: spec-lint's block scan continues past a blank line when no Falsifier line has been seen, so prose after a Draft requirement is scanned as requirement text; the body must end at the first blank line whether or not a falsifier was found
  - resolved: neither skill says when a decision made during the specification phase is recorded; DEC-003 requires a record at Judged and above, and a domain partition or a depth call is at least Judged
