...
findings:
  - open: an ambiguous Realized by identifier now reads as missing, because the batch reports ambiguity on standard error and the rewrite reads only standard output; the wake names build where LOOP-113 requires the repair "lengthen it", and no test holds the case
