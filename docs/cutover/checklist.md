...
Run by the developer, in /home/shawn/workspace2/sudus-dev, after the
...
/home/shawn/workspace2/sudus-dev at tag v0.7.0, v2 built from a shallow
tree copy of the sudus-v2 worktree); see the plan 14 report's "Fix round
...
- [ ] `.sudus/settings.json`'s `typesafeai` block has `weights`, `agent_ceiling`
      ...
      twice within v2's own development and a stale `.sudus/settings.json`
...
- [ ] `git -C /home/shawn/workspace2/sudus-dev status --porcelain` is empty
      and `git -C /home/shawn/workspace2/sudus-dev branch --show-current` is `main`.
- [ ] `git -C /home/shawn/workspace2/sudus-v2 status --porcelain` is empty.
- [ ] Look at the production checkout, /home/shawn/workspace2/sudus. It is
...
- [ ] Remove the `sudus-v2` worktree, so `v2` can be checked out directly
      in the main checkout below (Git refuses to check out a branch that
      is already checked out in another worktree of the same
      repository):

      cd /home/shawn/workspace2/sudus-dev
      git worktree remove /home/shawn/workspace2/sudus-v2

## Archive the 1.x line

    cd /home/shawn/workspace2/sudus-dev
    git fetch origin
    git tag -a v1-final -m "Final commit of the Sudus 1.x line before v2 became main" main
...
1.x commit, then move the tag to it:
    ...
    Sudus 1.x ends at tag v1-final. Sudus 2 continues on main; its
    records are commits on refs/sudus/log and refs/sudus/snapshots and
    1.x records under .sudus/ are not read (docs/spec/sudus-v2.md,
    section 12).

    git add README.md
    git commit -m "Close the 1.x line with a README note"
    git tag -f -a v1-final -m "Final commit of the Sudus 1.x line before v2 became main"
...
(`.sudus/evidence/`'s several thousand files among them) as ordinary
...
`sudus push`, not a plain `git push`, sends `main`, `refs/sudus/log` and
`refs/sudus/snapshots` together: the working agreement's own push
paragraph (lib/travel.mjs's AGREEMENT_PUSH_TEXT) says "Never push
refs/sudus/\* with plain git push", because a raw push has no lease and
sudus push validates each ref against the remote's own current tip
...
- [ ] docs/video/ is a local reference directory, never tracked in this
      ...
      /home/shawn/workspace2/sudus happens to have the files on disk,
...
|---|---|---|
| `.sudus/evidence/` | 7,608 | every mechanism's receipt history |
| `docs/decisions/` | 88 | 1.x decision files (the four-level scale) |
| `.sudus/reviews/` | 68 | independent review reports |
| `docs/commitments/` | 67 | 1.x commitment files |
| `.sudus/backlog/` | 43 | captured backlog items |
| `.sudus/stops/` | 27 | stop-hook refusal records |
| `.sudus/next-iteration/` | 14 | next-iteration items |
| `.sudus/escalations/` | 9 | escalation files |
| `docs/spec/*.md` (1.x) | 8 | autonomy.md, decisions.md, glossary.md, loop.md, overview.md, package.md, roadmap.md, specification.md -- replaced by v2's one docs/spec/sudus-v2.md |
| `docs/audit/` | 4 | audit reports |
| `docs/diagrams/` (1.x) | 3 | the 1.x loop diagrams |
| `.sudus/mechanisms/` | 3 | 1.x mechanism definitions (markdown, not v2's JSON) |
...
`.sudus/` under their own project (their evidence, reviews, escalations,
...
Once main is v2 and `sudus wake` on it reports `Done`, cut the release
...
- [ ] `sudus wake` on main prints `Done:`; `sudus push` succeeds against origin.
- [ ] The production checkout at /home/shawn/workspace2/sudus is not
      touched by this checklist; it fast-forwards from sudus-dev main
      only on your word, and eas4ai/sudus receives v2 only then.
