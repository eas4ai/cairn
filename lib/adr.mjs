...
// Fix round 2 (plan 11 review): reads settings through lib/settings.mjs's loadSettingsFull, the
// same calibration-aware load evaluate()/escalateWithRoute/preflight() now all use, so decide()
// (the only way escalateWithRoute's agent route completes) no longer refuses a route-mode project
// even with a genuinely passing calibration at the exact current policy digest.
export async function decide(cwd, { title, rests_on, wrong_if, body, by = 'agent', evaluation = null, named_paths = [] }, command = 'decide') {
  ...
  // circularity, one file over, that loadSettingsFull exists to work around; lib/snapshots.mjs is
  // outside this fix round's authorized files, so the already-loaded, calibration-aware `settings`
  // is passed through explicitly instead, the same way lib/evaluate.mjs's captureIdentity already
  // does.
  const base_snap = await writeWorkspaceSnapshot(cwd, { exclude: settings.network_exclude });
