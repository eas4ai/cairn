...
// queued another one. decisionFileBytes computes the validated line and the ADR file's new full
// bytes without writing anything, so the caller can stage it as an ordinary {store:'file'} write
// inside its own transaction plan: known before staging (this function's whole job), and replayed
// idempotently by applyWrites' existing file-write digest check on any recovery, the same as every
// other file write already is.
export async function decisionFileBytes(cwd, line, { command }) {
  const obj = await prepareLine(cwd, line, { command });
  const raw = await readRaw(cwd);
  return { id: obj.id, bytes: raw + canonicalize(obj) + '\n' };
}
