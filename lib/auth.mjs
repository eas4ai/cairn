...
export async function authorize(cwd, { sign, quote, decision = null, nonce, env } = {}) {
  ...
  if (!verifyEvidence(settings, evidence, { purpose: 'authorize', subject })) throw new AuthError('sudus: developer evidence does not verify');
...
export async function refuseUnauthorizedProtected(cwd, log) {
  ...
  // Revision 11: the digests are compared before the evidence is verified. The evidence verifies
  // against the settings on disk, which are the settings the record bound only while the digests
  ...
  if (!chainRecordVerifies(settings, last, rootSha)) {
    throw new AuthError("sudus: the latest authorization record's evidence does not verify against the current signing_key; ask the developer and record the answer with sudus authorize --quote <words>", 'unverified');
  }
}
