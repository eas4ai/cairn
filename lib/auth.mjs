...
//
// Settings with a key are always bound by signed evidence (an init with auth_mode 'signed', or a
// signed authorization), so an attested record's settings carry none and need no lookup. A signed
// record's settings are the file on disk when its digest still matches, else the version in Git
// history (the branch, or the workspace snapshots under refs/sudus/snapshots) with that digest.
...
async function boundKey(cwd, rec, disk) {
  const digest = rec.payload.settings_digest;
  disk ??= await loadSettings(cwd).catch(() => null);
  if (disk && disk.digest === digest) return disk.settings.signing_key ?? null;
  const signed = rec.kind === 'init' ? rec.payload.auth_mode === 'signed' : rec.payload.evidence?.mode === 'signed';
  if (!signed) return null;
  const settings = await settingsWithDigest(cwd, digest);
  if (!settings) throw new AuthError(`sudus: the settings in force (${digest}, bound by ${rec.sha}) are neither on disk nor in Git history; restore ${layoutOf(cwd).settings} to them`, 'unverified');
  return settings.signing_key ?? null;
}
...
export async function evidenceSettings(cwd, log) {
  const last = authorizations(log).at(-1);
  if (!last) return { signing_key: (await loadSettings(cwd)).settings.signing_key ?? null };
  return { signing_key: await boundKey(cwd, last) };
}
...
export async function authorize(cwd, { sign, quote, decision = null, nonce, env } = {}) {
  ...
  const signer = { signing_key: inForce.signing_key ?? settings.signing_key };
...
async function keysAround(cwd, chain, i) {
  try {
    return { before: i === 0 ? null : await boundKey(cwd, chain[i - 1]), after: await boundKey(cwd, chain[i]) };
...
export async function isAuthorized(cwd, path, beforeDigest, afterDigest) {
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    const keys = await keysAround(cwd, chain, i);
    if (!keys || !chainRecordVerifies(keys.before, keys.after, chain[i], rootSha)) continue;
...
export async function refuseUnauthorizedProtected(cwd, log) {
  ...
  const chain = authorizations(log);
  const keyBefore = chain.length > 1 ? await boundKey(cwd, chain.at(-2), disk) : null;
  if (!chainRecordVerifies(keyBefore, settings.signing_key ?? null, last, rootSha)) {
