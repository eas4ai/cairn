...
async function dirtyPaths(cwd, pathspecs) {
  const out = (await git(['status', '--porcelain=v1', '-z', '--', ...pathspecs], { cwd })).stdout;
...
export async function refuseUnauthorizedProtected(cwd, log) {
  ...
  if (!chainRecordVerifies(settings, last, rootSha)) {
    throw new AuthError("cairn: the latest authorization record's evidence does not verify against the current signing_key; run cairn authorize", 'unverified');
  }
  ...
  for (const cls of ['settings', 'agreement', 'spec']) {
    if (now[cls] !== bound[cls]) {
      throw new AuthError(`cairn: ${names[cls]} changed to ${now[cls]} without a developer authorization; run cairn authorize`, 'no-authorization');
    }
  }
}
