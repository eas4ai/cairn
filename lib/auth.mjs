...
import { layoutOf, SETTINGS_PATHS } from './layout.mjs';
...
export function harnessName(env = process.env) {
  if (env.SUDUS_HARNESS) return env.SUDUS_HARNESS;
...
async function gitAuthor(cwd) {
  ...
  if (r.code !== 0 || !m || !m[1] || !m[2]) throw new AuthError('sudus: attested evidence needs a Git author (user.name and user.email)');
...
// purpose.
export function requireQuote(purpose, quote) {
  if (typeof quote !== 'string' || quote.trim() === '') {
    throw new AuthError(`sudus: ${purpose} needs --quote <the developer's words>: quote what the developer said in the conversation, such as their ok`);
  }
}
...
// Revision 11: developer evidence verifies against the key in force, the signing_key of the
// settings the latest init or authorization bound, not the settings file on disk. Anyone who can
// write the working tree can change that file; checked against it, an agent could set its own key
// or none and then approve that change and every answer after it. While a key is in force, a
// settings change, including a new key or none, needs a signature from it.
//
// The key in force comes from walking the authorization chain from the log's first record, never
// from what a record says about itself: appendRecord is not developer-gated, so a record changes the
// key in force only when its evidence verifies against the key in force before it, or, with none in
// force, a signed record against the key of the settings it binds (turning a key on takes that key's
// signature). A forged record verifies against neither and changes nothing. An attested chain needs
// no settings lookup; a record's settings are otherwise the file on disk when its digest still
// matches, else the version in Git history (the branch, or the workspace snapshots under
// refs/sudus/snapshots) with that digest.
async function settingsWithDigest(cwd, digest) {
  const r = await git(['log', '--all', '--format=%H', '--', ...SETTINGS_PATHS], { cwd });
  const seen = new Set();
  for (const commit of r.stdout.split('\n').filter(Boolean)) {
    for (const p of SETTINGS_PATHS) {
      const b = await git(['rev-parse', '--verify', '-q', `${commit}:${p}`], { cwd, expect: [0, 1, 128] });
      const blob = b.stdout.trim();
      if (b.code !== 0 || seen.has(blob)) continue;
      seen.add(blob);
      let settings;
      try { settings = JSON.parse((await git(['cat-file', 'blob', blob], { cwd })).stdout); } catch { continue; }
      if (sha256(canonicalize(settings)) === digest) return settings;
    }
  }
  return null;
}

// The signing_key of the settings `rec` bound, or undefined when no copy of them can be found.
async function keyOf(cwd, rec, disk) {
  const digest = rec.payload.settings_digest;
  if (disk && disk.digest === digest) return disk.settings.signing_key ?? null;
  const settings = await settingsWithDigest(cwd, digest);
  return settings ? settings.signing_key ?? null : undefined;
}

function missingSettings(cwd, rec) {
  return new AuthError(`sudus: the settings in force (${rec.payload.settings_digest}, bound by ${rec.sha}) are neither on disk nor in Git history; restore ${layoutOf(cwd).settings} to them`, 'unverified');
}

// before[i] is the key in force before chain[i], valid[i] whether chain[i] verified, and key the key
// in force after the whole chain. Only the log's first record counts as the init record.
async function keyChain(cwd, log, disk) {
  ...
  for (const rec of chain) {
    before.push(key);
    if (rec.kind === 'init') {
      const root = rec.sha === rootSha;
      valid.push(root);
      if (root && rec.payload.auth_mode === 'signed') {
        key = await keyOf(cwd, rec, disk);
        if (key === undefined) throw missingSettings(cwd, rec);
      }
      continue;
    }
    const ev = rec.payload.evidence;
    const expect = { purpose: 'authorize', subject: canonicalize({ spec: rec.payload.spec_digest, agreement: rec.payload.agreement_digest, settings: rec.payload.settings_digest }) };
    let ok;
    if (key !== null) {
      ok = verifyEvidence({ signing_key: key }, ev, expect);
      if (ok) {
        key = await keyOf(cwd, rec, disk);
        if (key === undefined) throw missingSettings(cwd, rec);
      }
    } else if (ev?.mode === 'signed') {
      const after = await keyOf(cwd, rec, disk);
      ok = typeof after === 'string' && verifyEvidence({ signing_key: after }, ev, expect);
      if (ok) key = after;
    } else {
      ok = verifyEvidence({ signing_key: null }, ev, expect);
    }
    valid.push(ok);
  }
  return { chain, before, valid, key };
}

// The settings object to authenticate and verify a developer record against, written after `log`.
// Before init nothing is in force, and the settings file on disk decides.
export async function evidenceSettings(cwd, log, disk) {
  disk ??= await loadSettings(cwd);
  if (!authorizations(log).length) return { signing_key: disk.settings.signing_key ?? null };
  return { signing_key: (await keyChain(cwd, log, disk)).key };
}
...
export async function authorize(cwd, { sign, quote, decision = null, nonce, env } = {}) {
  ...
  if (!log.some((r) => r.kind === 'init')) throw new AuthError('sudus: run sudus init first');
  if (!existsSync(join(cwd, 'AGENTS.md'))) throw new AuthError('sudus: AGENTS.md is missing; authorize binds the working agreement');
  if (!existsSync(join(cwd, 'docs/spec'))) throw new AuthError('sudus: docs/spec is missing; authorize binds the specification');
  ...
  // With a key in force, it signs; with none, the settings being bound decide, so setting a key
  // needs a signature from the new key (revision 11; evidenceSettings above).
  const inForce = await evidenceSettings(cwd, log);
  const signer = { signing_key: inForce.signing_key ?? settings.signing_key ?? null };
  const evidence = await authenticateDeveloper(cwd, signer, { purpose: 'authorize', subject, sign, quote, nonce, env });
  if (!verifyEvidence(signer, evidence, { purpose: 'authorize', subject })) throw new AuthError('sudus: developer evidence does not verify');
  const dirty = await dirtyPaths(cwd, [layoutOf(cwd).settings, 'AGENTS.md', 'docs/spec']);
...
export async function direction(cwd, { kind, quote, env } = {}) {
  if (kind !== 'instead' && kind !== 'ask') throw new AuthError('sudus: authorize takes ok, instead or ask');
...
export function protectedClass(p) {
  if (SETTINGS_PATHS.includes(p)) return 'settings';
...
// all since the init schema carries none) made isAuthorized('.sudus/settings.json', before, after)
// return true, because that forged record was accepted as a second chain root. The one legitimate
// init record is refs/sudus/log's first record and nothing else (sections 3 and 4); a later record
...
// Revision 11: a record's evidence verifies against the key in force before it (keyBefore), and
// only when there was none against the key of the settings it bound (keyAfter), as authorize writes it.
function chainRecordVerifies(keyBefore, keyAfter, rec, rootSha) {
  ...
  return verifyEvidence({ signing_key: keyBefore ?? keyAfter }, rec.payload.evidence, { purpose: 'authorize', subject });
}
...
export async function isAuthorized(cwd, path, beforeDigest, afterDigest) {
  ...
  const log = await readLog(cwd);
  let walk;
  try { walk = await keyChain(cwd, log, await loadSettings(cwd).catch(() => null)); }
  catch (e) { if (e instanceof AuthError) return false; throw e; }
  const { chain, valid } = walk;
...
export async function refuseUnauthorizedProtected(cwd, log) {
  ...
  if (!bound) throw new AuthError('sudus: run sudus init first', 'no-authorization');
  // Fix round 1, item 2: re-verify the record that bound the current state; a forged newest record
  // refuses whether or not now/bound happen to agree (below, after the digest comparison).
  const last = authorizations(log).at(-1);
  const disk = await loadSettings(cwd);
  const { settings } = disk;
  const rootSha = rootInitSha(log);
  ...
  if (last.kind === 'init' && last.sha !== rootSha) {
    throw new AuthError(`sudus: ${last.sha} is a second record of kind init on refs/sudus/log; the init record is the log's first record and nothing else; this is a breach`, 'breach');
  }
  // Revision 11: the digests are compared before the evidence is verified. The key after the latest
  // record is read from the settings on disk, which are the settings it bound only while the digests
  // agree; after a developer sets signing_key, the attested record before it would otherwise read as
  // unverifiable instead of naming the settings change to authorize. Either check still refuses.
  const now = await protectedDigests(cwd);
  const names = { spec: 'docs/spec', agreement: 'AGENTS.md', settings: layoutOf(cwd).settings };
  for (const cls of ['settings', 'agreement', 'spec']) {
    if (now[cls] !== bound[cls]) {
      throw new AuthError(`sudus: ${names[cls]} changed to ${now[cls]} without a developer authorization; ask the developer and record the answer with sudus authorize --quote <words>`, 'no-authorization');
    }
  }
  const { before } = await keyChain(cwd, log, disk);
  if (!chainRecordVerifies(before.at(-1), settings.signing_key ?? null, last, rootSha)) {
    throw new AuthError("sudus: the latest authorization record's evidence does not verify against the signing key in force; ask the developer and record the answer with sudus authorize --quote <words>", 'unverified');
  }
}
...
export async function readDecision(cwd, id, { sign, quote, nonce, env } = {}) {
  if (!ULID.test(id)) throw new AuthError('sudus: decision id must be a 26-character ULID');
  await loadSettings(cwd);
  const settings = await evidenceSettings(cwd, await readLog(cwd));
  const evidence = await authenticateDeveloper(cwd, settings, { purpose: 'read', subject: id, sign, quote, nonce, env });
  if (!verifyEvidence(settings, evidence, { purpose: 'read', subject: id })) throw new AuthError('sudus: developer evidence does not verify');
...
export function cliSigner(argv, io) {
  ...
  const given = i >= 0 ? argv[i + 1] : (io.env.SUDUS_SIGNATURE ?? io.env.CAIRN_SIGNATURE);
  const nonce = cliNonce(argv);
  if (!given) {
    return async (bytes) => {
      io.stdout(`sudus: sign this payload: ${Buffer.from(bytes).toString()}`);
      io.stdout('sudus: e.g. openssl pkeyutl -sign -rawin -inkey dev.pem | base64url');
      throw new AuthError("sudus: signing_key is set; pass --signature or SUDUS_SIGNATURE, with --nonce <n> matching the payload's nonce printed above");
    };
  }
  ...
  return async (bytes) => {
    ...
    if (nonce === null || !payload || payload.nonce !== nonce) {
      throw new AuthError('sudus: --nonce must match the nonce in the payload being signed; pass --nonce <n> together with --signature');
    }
...
export function runDecisionsRead(argv, io) {
  return refusing(io, async () => {
    ...
    io.stdout(`sudus: read ${id} recorded as ${sha} (${describeEvidence(rec.payload.evidence)})`);
    return 0;
  });
}
...
export function runAuthorize(argv, io) {
  return refusing(io, async () => {
    ...
    if (word === undefined || word === 'ok') {
      ...
      const rec = (await readLog(io.cwd)).find((r) => r.sha === sha);
      io.stdout(`sudus: authorization ${sha} (${describeEvidence(rec.payload.evidence)})`);
      return 0;
    }
    const sha = await direction(io.cwd, { kind: word, quote, env: io.env });
    io.stdout(`sudus: direction ${sha} (${word}: "${quote}")`);
    return 0;
  });
}
...
export async function authenticateDeveloper(cwd, settings, { purpose, subject, sign, quote, nonce, env } = {}) {
  ...
  if (settings.signing_key !== null && settings.signing_key !== undefined) {
    if (typeof sign !== 'function') throw new AuthError('sudus: signing_key is set; pass --signature or SUDUS_SIGNATURE');
    ...
    if (!verifySigned(settings.signing_key, ev)) throw new AuthError('sudus: the signature does not verify against signing_key');
    return ev;
  }
