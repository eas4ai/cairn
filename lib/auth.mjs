...
export async function authorize(cwd, { sign, quote, decision = null, nonce, env } = {}) {
  ...
  const dirty = await dirtyPaths(cwd, ['.sudus/settings.json', 'AGENTS.md', 'docs/spec']);
...
export function protectedClass(p) {
  if (p === '.sudus/settings.json') return 'settings';
...
export async function refuseUnauthorizedProtected(cwd, log) {
  ...
  const names = { spec: 'docs/spec', agreement: 'AGENTS.md', settings: '.sudus/settings.json' };
...
export function cliSigner(argv, io) {
  ...
  const given = i >= 0 ? argv[i + 1] : io.env.SUDUS_SIGNATURE;
