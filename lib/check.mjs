...
// later written to `.cairn/output/` by writeOutput) at OUTPUT_CAP bytes, appending one marker line
...
export function runCommand(cwd, command, { cap = OUTPUT_CAP } = {}) {
  return new Promise((resolve) => {
    ...
    const push = (c) => {
      ...
      chunks.push(Buffer.from(`cairn: output truncated at ${cap} bytes\n`));
      truncated = true;
    };
    let child;
    try {
...
async function writeOutput(cwd, bytes) {
  ...
  // by the next check.
  const gitignore = join(cwd, OUTPUT_DIR, '.gitignore');
...
export async function check(cwd, REQ) {
  ...
  if (before.needed) throw new CheckError(`${before.tried} distinct failing attempts at ${REQ} without a pass; a fourth attempt needs an escalation first: cairn escalate --commitment <slug> --concern ${REQ} ...`);
...
export async function isCurrent(cwd, receipt, REQ, now) {
  try { now = now ?? await identitiesNow(cwd, REQ); } catch (e) { if (expected(e)) return false; throw e; }
  let commit;
