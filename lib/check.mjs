...
export function attemptState(log, REQ) {
  ...
  const { count: tried, third } = attemptWalk(log, REQ, { since: r.start.sha, siblings: set });
...
// last pass, or -1.
function attemptWalk(log, REQ, { since = null, siblings = [] } = {}) {
  const seen = new Set();
  let counting = since === null, third = -1;
  log.forEach((rec, i) => {
    if (!counting) { if (rec.sha === since) counting = true; return; }
