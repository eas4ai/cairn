...
async function measureCommand(argv, { cwd, stdout, stderr, env }) {
  ...
  if (r.pending === 'review') {
    ...
    // Mirrors brief()'s own `.cairn/output/brief-<digest>.md` (lib/review.mjs): the brief text is
    ...
    // naming the harness, model, transport and the brief file as the fresh reviewer's entire
    // prompt -- no working directory, since M(D) already carries everything (renderMeasureBrief's
    // own comment) -- closing with the follow-up command. The intent's own short sha sits between
    // the slug and --file purely as a human-readable reference to which pending measurement this
    // answers; measureCommand's own <slug> --file <path> branch above reads only argv[0] as the
    // slug, so an extra token there is inert even if the line is pasted verbatim.
    const block = [
      `cairn: measure ${draft.commitment} ${r.intentSha} pending review`,
      `brief: ${briefPath}`,
      `harness: ${launch.name}`,
      `model: ${any(launch.model)}`,
      `transport: ${any(launch.transport)}`,
      `boundary: ${launch.boundary}`,
      `start: in ${launch.name}, start a fresh reviewer with model ${any(launch.model)} over ${any(launch.transport)}, with the file ${briefPath} as its entire prompt; when it finishes, run: cairn measure ${r.slug} ${r.intentSha.slice(0, 7)} --file <path to its answers>`,
      '',
    ].join('\n');
    stdout.write(`${block}\n${text}\n`);
    return 0;
  ...
  // nothing to show there too, for a different reason. Worded to be true either way.
  if (brief.present) { stderr.write('cairn: measure: --brief has nothing to show; this measurement already finished\n'); return 1; }
