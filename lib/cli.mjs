...
// Spec revision 6: the developer is never asked to run a command, so the confirm/confirmRemote/
// chooseKey/confirmDigest terminal-backed overrides this used to pass through are gone; every
// developer-only command instead reads --quote off argv (lib/auth.mjs's own flagValue-style
// readers) and takes `env` here, defaulting to process.env, threaded through to
// authenticateDeveloper for harness detection (attested evidence). lib/init.mjs's own
// confirmRemote/chooseKey/confirmDigest questions are unaffected here (a later task moves them to
// flags too); with no override coming through ctx any more, `cairn init` falls back to its
// terminal-backed defaults exactly as a real invocation always did.
...
// developer-only command's attested evidence); lib/init.mjs's own confirmRemote/chooseKey/
// confirmDigest are no longer threaded through toLineIo, so `cairn init` always falls back to its
// terminal-backed defaults, exactly as a real cairn.mjs invocation always did.
