...
import { applyTouch, reviewMechanism, readMechanisms } from './mechanisms.mjs';
...
import { cmdWake, FETCH_LINE as wakeFetchLine } from './wake.mjs';
...
// Fix round 1, item 9(b): this placeholder line named an unresolved `<authority>` remote, while
// lib/wake.mjs's own FETCH_LINE(remote) already builds the real, two-line section 4 form with the
// project's actual configured remote. When one is configured, requireRefs now uses that real
// form; tests/cli.test.mjs's own "show exits 3 ... when the durable refs are missing" test builds
// its fixture with makeRepo() (no .cairn/settings.json at all, so no remote can be read), so the
// placeholder above is kept as the fallback for that case rather than changed to wake's own
// "cairn: missing X; run cairn init" text, which would also be a different message for the same
// fixture and break that existing, unrelated assertion.
export async function requireRefs(cwd) {
  ...
  if (!(await readRef(cwd, LOG_REF)) || !(await readRef(cwd, SNAPSHOTS_REF))) {
    const remote = (await loadSettings(cwd).catch(() => null))?.settings.authority_remote;
    throw new NoVerdict(`durable refs missing; run: ${remote ? wakeFetchLine(remote) : FETCH_LINE}`);
  }
}
...
//
// Plan 11: escalateCommand now routes through lib/escalate.mjs's escalateWithRoute (already
// committed), which dynamic-imports evaluate from lib/evaluate.mjs and, when the project's
// typesafeai.enabled is true, may convert this Consequential-shaped draft into a queued agent
// decision or a capture instead of a held escalation (spec section 10; decision 23: "cairn
// escalate writes the evaluation route it selected"). With the evaluator disabled,
// escalateWithRoute's own fallback is a plain escalation -- the same behavior the old
// cliEscalate/escalate() path always had, so this is a strict superset, not a behavior change for
// a project that never turns the evaluator on. `cairn decide --consequential` (decideCommand,
// above) is unaffected: lib/escalate.mjs's decideConsequential (already committed and tested:
// "writes the same line without an evaluation") never consults the evaluator, so its flag surface
// and behavior stay exactly as they were; only `cairn escalate` gained a route.
//
// `--transport-module <path>` is test-only (named in docs/plans/overview.md's shared interfaces
// and plan 14's fixtures): when present, its default export replaces bin/typesafeai.mjs's post()
// as the network call the evaluator makes, so a test can prove no socket opens. It is stripped
// from argv before parseEscalateArgs sees the rest, since it is not one of the draft's own fields.
const TRANSPORT_FLAG = '--transport-module';
function splitTransportFlag(argv) {
  const i = argv.indexOf(TRANSPORT_FLAG);
  if (i < 0) return { rest: argv, transportPath: undefined };
  return { rest: [...argv.slice(0, i), ...argv.slice(i + 2)], transportPath: argv[i + 1] };
}
async function escalateCommand(argv, { cwd, stdout }) {
  const { rest, transportPath } = splitTransportFlag(argv);
  const draft = parseEscalateArgs(rest);
  const transport = transportPath ? (await import(pathToFileURL(resolvePath(transportPath)).href)).default : undefined;
  const { escalateWithRoute } = await import('./escalate.mjs');
  const r = await escalateWithRoute(cwd, draft, { transport });
...
export const COMMANDS = {
  ...
  escalate: { usage: 'escalate --commitment <s> --concern <token>... --question <q> --recommendation <r> --because <b> --if-wrong <w> --instead <i> [--option <t>...] [--path <p>...] [--decision <id>...] [--transport-module <path> (test only)]', run: escalateCommand },
