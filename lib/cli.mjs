...
// and plan 14's fixtures): when present, its default export was to replace bin/typesafeai.mjs's
// post() as the network call the evaluator makes, so a test can prove no socket opens. It is
// still stripped from argv before parseEscalateArgs sees the rest (an unrecognized flag would
// otherwise refuse), but escalateWithRoute makes no call of its own to hand it to until plan 16
// wires measure() in; the resolved `transport` module below is presently unused.
