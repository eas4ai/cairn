...
// `cairn show items`: one line per item record, oldest first, with its sha (what `cairn promote`
...
async function showItems(cwd, stdout) {
  ...
  for (const r of log) {
    ...
    const state = promoted ? ' promoted' : fixed ? ' fix recorded' : '';
...
// are `cairn init`'s own flags (--remote/--local-only/--signing-key/--attested/--adopt), read off
...
//
// Plan 16, Task 4 (correction to plan on record, brief's own text): `cairn decide --consequential`
...
// module imported it -- lib/escalate.mjs is one of this task's own files). `cairn escalate`
...
// `route:` word in the printed line, since there is no route left to report. `cairn escalate
...
// composite -- can take. `cairn decide --consequential` (decideCommand, above) dispatches on its
...
// escalateConsequential makes a model call of their own. Plan 16 (Task 5) gives `cairn measure`
...
// Plan 11: `cairn calibrate` records a calibration record over labelled suggested-agent
...
// the same pass/fail-as-exit-code convention `cairn lint` already uses.
//
...
// Plan 16, Task 5: `cairn measure` -- runs `measure()` (lib/evaluate.mjs) on the exact draft.
...
async function measureCommand(argv, { cwd, stdout, stderr, env }) {
  ...
  if (rest[0] && !rest[0].startsWith('--')) {
    // cairn measure <slug> --file <path>: complete a pending review measurement. Built by Task 6
  ...
  // Fix (Important I3, final-review.md): `cairn brief` accepts `[--harness <name>]` (lib/review.mjs's
  // cliBrief, reading named.harness) and section 10 says the review source starts the adversary
  // "exactly as cairn brief" does -- but measureCommand never read one off argv, leaving
  ...
  const r = await measure(cwd, draft, { transport, session: process.env.CAIRN_SESSION ?? null, harness: harnessFlag.value, env: env ?? process.env });
  if (r.pending === 'review') {
    ...
    // brief() (lib/review.mjs) already owns for cairn brief, split across two files for parallel
  ...
  // finished."
  if (brief.present) { stderr.write(`cairn: measure: --brief has nothing to show; ${r.reason}\n`); return 1; }
  ...
  stdout.write([
    `cairn: measure ${draft.commitment} ${r.measurementSha} ${r.outcome}${r.suggested ? ` suggested:${r.suggested}` : ''}`,
    `levels: ${levels}`,
...
// reads --signature/CAIRN_SIGNATURE and prints the payload to sign when neither is present;
...
async function checkCommand([REQ], { cwd, stdout, stderr }) {
  ...
  // A per-requirement result that is not exactly `cairn: REQ: pass|fail` lands on unverified;
  ...
  for (const x of rec.payload.results) {
    ...
    stderr.write(`cairn: ${x.requirement} is unverified: no output line is exactly "cairn: ${x.requirement}: pass" or "cairn: ${x.requirement}: fail"; ${saw}\n`);
  }
}
...
async function reviewMechanismCommand([req, receipt], { cwd, stdout, stderr }) {
  if (!req) { stderr.write('cairn: review mechanism needs <REQ> [<fail-receipt>]\n'); return 1; }
  if (!receipt) {
    ...
    if (!fr) { stderr.write(`cairn: no fail receipt for ${req}; run cairn check ${req} against a violating example first\n`); return 1; }
    receipt = fr.sha;
  }
...
export const COMMANDS = {
  ...
  init: { usage: 'init --remote <name>|--local-only --signing-key <path>|--attested [--adopt <digest>] --quote <words>', run: initCommand },
  ...
  decide: { usage: 'decide --consequential --title <t> --rests-on <REQ,...> --wrong-if <t> --body <t> (a spec-phase deference decision) | decide --consequential --commitment <s> --concern <token>... --question <q> --recommendation <r> --because <b> --if-wrong <w> --instead <i> [--option <t>...] [--path <p>...] [--decision <id>...] (a measured work-loop decision)', run: decideCommand },
  realize: { usage: 'realize <decision-id> --subject <text>', run: realizeCommand },
  escalate: { usage: 'escalate [--consequential] --commitment <s> --concern <token>... --question <q> --recommendation <r> --because <b> --if-wrong <w> --instead <i> [--option <t>...] [--path <p>...] [--decision <id>...]', run: escalateCommand },
  calibrate: { usage: 'calibrate', run: calibrateCommand },
  measure: { usage: 'measure [--brief] [--harness <name>] --commitment <s> --concern <token>... --question <q> --recommendation <r> --because <b> --if-wrong <w> --instead <i> [--option <t>...] [--path <p>...] [--decision <id>...] [--transport-module <path> (test only)] | measure <slug> --file <path>', run: measureCommand },
...
export function usage() {
  return ['usage: cairn <command> [args]', '', ...Object.values(COMMANDS).map((c) => `  cairn ${c.usage}`), '  cairn --help', '  cairn --version', ''].join('\n');
}
...
// developer-only command's attested evidence); `cairn init` takes its own answers as argv flags,
...
export async function main(argv, { cwd = process.cwd(), stdout = process.stdout, stderr = process.stderr, ...inject } = {}) {
  ...
  try {
    ...
    // Every path Cairn handles (declared inputs, leases, snapshots, breaches) is repository-relative,
    // so a command typed in a subdirectory runs at the top level; `cairn wake` from scripts/x used
    ...
    // `cairn <command> --help` prints that command's usage line and runs nothing. Only the first
    ...
    if (args[0] === '--help' || args[0] === '-h') { stdout.write(`usage: cairn ${cmd.usage}\n`); return 0; }
