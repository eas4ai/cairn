...
// decision or a capture -- that router was the design decisions 55 and 56 superseded.
//
// Plan 16, Task 4: escalateWithRoute is retired from this command's own path (it stays in
// lib/escalate.mjs, unused here, since removing it outright is outside this task's named files).
// `cairn escalate` without `--consequential` now calls lib/escalate.mjs's plain escalate() --
// exactly the pre-plan-11 behavior escalateWithRoute's own 'developer' route always produced, just
// with no `route:` word in the printed line, since there is no route left to report. `cairn
// escalate --consequential` is new: it calls escalateConsequential (Task 4, this plan), the only
// path a floor-caught or vetoed draft -- or an agent's own choice to escalate past a `suggested:
// agent` composite -- can take. `cairn decide --consequential` (decideCommand, above) dispatches
// on its own flag set (--title vs --commitment) rather than being affected by this change.
//
