...
// decideCommand calls lib/adr.mjs's decide() directly from its own --title/--rests-on/--wrong-if/
// --body flags, never lib/escalate.mjs's decideConsequential, so its flag surface and behavior
// stay exactly as they were. (Plan 16, task 3: decideConsequential itself -- reachable today only
// by calling it directly, as tests/escalate.test.mjs does, not through this CLI command -- now
// requires a current, composite-outcome measurement via lib/evaluate.mjs's currentMeasurement and
// names it on the decision line's `evaluation` field; it no longer reads the draft's own optional
// `evaluation` field at all.)
//
