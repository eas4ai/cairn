...
import { cliEscalate, cliAnswer, cliReply, cliDispute } from './escalate.mjs';
...
// cliEscalate/cliAnswer/cliReply/cliDispute keep that {code, out} contract, since their own tests
// call them directly; these adapters translate one shape into the other, on out on 0 and stderr on 1.
async function escalateCommand(argv, { cwd, stdout, stderr }) {
  const { code, out } = await cliEscalate(cwd, argv);
