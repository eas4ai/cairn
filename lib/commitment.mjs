...
export async function fix(cwd, itemSha) {
  ...
  if (!open) {
    // A defect against a commitment's own requirement is worked under it and blocks Done, and
    // cairn done refuses while one is unfixed. A done record written before that check existed
    // (through 2.1.13) left the defect with no command that could discharge it: wake named fix,
    // fix wanted an open commitment. The last commitment still owns the defect it raised against
    // its own requirement, so the fix is recorded against that commitment's start.
    const last = log.filter((r) => r.kind === 'start').at(-1);
    const owns = last && last.payload.requirements.some((q) => q.requirement === it.payload.source);
    if (!owns) refuse(`a fix is recorded under an open commitment whose requirement ${it.payload.source} is; open the next commitment with /next-feature, then cairn fix ${itemSha}`);
    open = last;
  }
