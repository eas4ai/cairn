...
export async function fix(cwd, itemSha) {
  ...
  if (open) {
    ...
    // recordable with nothing open. The protected-delta base is the contract in force now: the
    // last commitment's done snapshot (issue #4: its start snapshot predates the spec edits that
    // commitment made and its done authorized, so a fix was refused for the commitment's own changes).
    if (fixBase(log) === null) refuse('a fix is recorded after a first commitment has started');
  }
  const snapshot = await writeWorkspaceSnapshot(cwd);
  const changed = await protectedDelta(cwd, open ? open.payload.snapshot : fixBase(log), snapshot);
