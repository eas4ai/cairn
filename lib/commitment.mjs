...
import { refuseUnauthorizedProtected, authorizations, authenticateDeveloper, verifyEvidence } from './auth.mjs';
...
// protectedDigests is deliberately its own implementation here, not a re-export of lib/auth.mjs's
// function of the same name (Task 3's own Interfaces block lists it as an export of this module,
// and does not list lib/auth.mjs among Task 3's consumed interfaces). It follows the same rule
// lib/auth.mjs's specDigest/agreementDigest already establish (spec section 2: "the roadmap is
// bound structurally rather than by digest"), so the two independent implementations must and do
// produce identical digests for identical repository state -- exercised directly by the Task 4
// test 'the authorization record carries the three digests by the shared rule'.
...
// Deviation from the plan text: the same withTransaction-contract note as supersede's own
// (writeStart's comment above) applies here too, plus one more wrinkle specific to promote. The
// 'promotion' record must be written to refs/cairn/log *before* the terminal 'start' record inside
// the same transaction (the test finds it by scanning the log for the last 'promotion' record,
// positioned before log.at(-1), the successor start), and it should carry the same intent as that
// terminal record -- but a plan.writes 'log' entry's payload, like a 'file' write's bytes, must be
// fully known before staging, before the transaction's own command-intent (and its intentSha) is
// created. `fn` runs after applyWrites but before the terminal record is appended, and it does
// receive intentSha, so 'promotion' is written directly inside fn with that real intent value; fn
// returns null so the terminal 'start' payload comes from plan.terminal.payload (with `snapshot`
// resolved from the plan's own 'snapshot' write) exactly as in writeStart.
export async function promote(cwd, itemSha, { installRefspecs = noop } = {}) {
  ...
  const plan = {
    ...
    writes: [{ store: 'branch', paths, message: `Promote ${slug}` }, { store: 'snapshot' }],
    terminal: { kind: 'start', target: slug, payload: { slug, snapshot: { stepRef: 1 }, requirements: prepared.set, from_superseded: null } },
  };
  const r = await withTransaction(cwd, { command: 'promote', plan }, async ({ intentSha }) => {
    await appendRecord(cwd, 'promotion', slug, { item: itemSha, decision, intent: intentSha, results: [] });
    return null;
  });
  if (prepared.settings.authority_remote !== null) await installRefspecs(cwd, prepared.settings.authority_remote);
  return r.terminalSha;
