...
async function findItem(log, ref) {
  ...
  if (!it) refuse(`${ref} is not an item record or item slug; cairn show items lists them`);
  return it;
}
...
export async function fix(cwd, itemSha) {
  ...
  if (open) {
    ...
    // recordable with nothing open. The protected-delta base is the last commitment's start.
    open = log.filter((r) => r.kind === 'start').at(-1);
    if (!open) refuse('a fix is recorded after a first commitment has started');
  }
...
// `cairn authorize` can solve. Only a genuine "nothing to verify" or "no longer matches" state maps
...
async function prepareStart(cwd, slug, log) {
  ...
  if (!(await currentAuthorization(cwd, log))) refuse('no current authorization binds the specification, working agreement and settings; ask the developer and record the answer with cairn authorize --quote <words>');
  const { settings } = await loadSettings(cwd);
...
// supersede) without building a commitment to Done; cairn done itself never passes it.
...
export async function promote(cwd, itemSha, { installRefspecs = installRefspecsReal, failAfterWrite = -1 } = {}) {
  ...
  if (current !== null && current !== slug && current !== finished) refuse(`roadmap names ${current} as Current:, not the finished commitment ${finished} or ${slug}; the developer's next section is started with cairn start ${current}, not promoted`);
  const base_snap = await writeWorkspaceSnapshot(cwd);
...
// fix round) checks the write against the local cairn-managed ledger (recordManagedWrite) or an
  ...
  if (stops.length) {
    ...
    if (!approved) {
      ...
      const sha = await writeEscalation(cwd, slug, {
        ...
        instead: 'revise the realization and run cairn realize again', concerns: [token],
      });
      throw new RealizationError(`realization of ${decisionId} touches ${list}; the decision is the developer's; escalation ${sha} written, their ok lets cairn realize proceed`, stops);
    }
