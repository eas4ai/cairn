...
export async function writeCycleEscalation(cwd, slug, bound) {
  ...
  const what = bound.kind === 'acceptanceRounds' ? `${BOUNDS.acceptanceRounds} acceptance rounds after the report have not reached Done`
    ...
    : bound.kind === 'liveness' ? `${bound.actionClass} would write ${bound.target} and create a ${bound.violation} violation${bound.because ? ` (${bound.because})` : ''}`
...
export async function guardKernelWrite(cwd, path, bytes, { action }) {
  ...
  await writeCycleEscalation(cwd, slug, { kind: 'liveness', actionClass: action, target: path, violation: refuseAs, because });
  throw new LivenessError(`cairn: writing ${path} for ${action} would create a ${refuseAs} violation${because ? ` (${because})` : ''}; resolve that first, then run the command again; cycle escalation written`);
}
