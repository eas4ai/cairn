...
import { isMechanismDefinition } from './layout.mjs';
import { canonicalize, parseStrict } from './canon.mjs';
import { readLog } from './records.mjs';
import { progressSummary, progressMade, doneRule, readState, verdictOf, ORDER, predicates } from './wake.mjs';
import { runWithPreflight } from './scope.mjs';
import { writeEscalation } from './escalate.mjs';
import { openFindingConcerns } from './review.mjs';
...
const counterFile = (cwd) => gitPath(cwd, 'sudus-cycle.json');
...
export async function readCounter(cwd) {
  ...
  catch (e) { throw new Error(`sudus: ${file} is corrupt: ${e.message}`); }
}

// Fix round 2 finding 15 (plan 09 re-review): a fixed `<file>.tmp` name let two concurrent
// writers collide -- whichever renamed second found its own .tmp already gone (ENOENT), even
// though its own write had just succeeded, so a concurrent state-changing command could write
// its record, print success, and still exit 1 on an unrelated rename failure. Unique per writer
// (a random suffix; pid alone is not enough since concurrent in-process callers, as in main()'s
// own withLoop, share one pid) so concurrent writers never step on each other's temp file. The
// final rename target is unchanged, so "last writer wins" for the counter's own value is
// unchanged too -- this only fixes the crash, not the counter's own best-effort race.
export async function writeCounter(cwd, counter) {
  const file = await counterFile(cwd);
  const tmp = `${file}.tmp.${process.pid}.${randomBytes(4).toString('hex')}`;
  await writeFile(tmp, canonicalize(counter));
...
export function openCycleEscalation(log) {
  return log.find((r) => r.kind === 'escalation' && r.payload.concerns.split(' ').includes('cycle') && !log.some((a) => a.kind === 'answer' && a.payload.escalation === r.sha && a.payload.kind !== 'ask')) ?? null;
}
...
// concerns are the fixed 'cycle' token, not a checked one, plus the findings an acceptance-round
// escalation names, which come from the ledger).
export async function writeCycleEscalation(cwd, slug, bound) {
  const log = await readLog(cwd);
  const open = openCycleEscalation(log);
  if (open) return open.sha;                                    // one escalation per cycle, never one per completion
  // Issue #23: rounds that each end with a finding open can run on without bound, and ok alone
  // only restarts the count. The escalation names the open findings and recommends closing them,
  // each captured as a backlog item; the developer's ok closes them (lib/escalate.mjs disputes),
  // so Done needs no further round. Another round or a supersession is the instead answer.
  const findings = bound.kind === 'acceptanceRounds' ? openFindingConcerns(log, slug) : [];
  if (findings.length > 0) {
    const names = findings.map((f) => `finding ${f.n} on the ${f.kind} ${f.source.slice(0, 12)}`).join(', ');
    return writeEscalation(cwd, slug, {
      question: `The loop is cycling: ${BOUNDS.acceptanceRounds} acceptance rounds after the report have not reached Done, and ${names} ${findings.length === 1 ? 'is open. Does it' : 'are open. Do they'} need another round?`,
      recommendation: 'Close the open findings as answered and capture each as a backlog item, so Done needs no further acceptance round',
      because: `each of the last ${BOUNDS.acceptanceRounds} acceptance rounds ended short of Done, and another round is likely to end the same way`,
      if_wrong: 'a defect one of these findings names ships unfixed until its backlog item is worked',
      instead: 'take another round, which restarts the count, or supersede the commitment',
      concerns: ['cycle', ...findings.map((f) => f.token)],
    });
  }
  const what = bound.refused ? `${bound.actionClass} ${bound.target} was refused ${bound.refused} times: it would create a ${bound.violation} violation${bound.because ? ` (${bound.because})` : ''}`
    : bound.kind === 'acceptanceRounds' ? `${BOUNDS.acceptanceRounds} acceptance rounds after the report have not reached Done`
...
// Rounds are counted after the report and after the developer's last ok or instead answer to a
// cycle escalation, so the escalation's own "the count is reset by an ok answer" is what the
// code does. It used to count every round after the report forever: once three existed, each ok
// closed the open escalation and the next settle wrote a new one, an escalation per round.
// accept() (lib/review.mjs) counts through this too; its own copy counted from the report, so the
// first round after an ok wrote a new cycle escalation.
export function acceptanceRounds(records, log) {
  const rep = records.filter((r) => r.kind === 'report').at(-1);
  if (!rep) return 0;
  const after = records.slice(records.indexOf(rep) + 1);
  let from = 0;
  after.forEach((r, i) => {
    if (r.kind !== 'answer' || r.payload.kind === 'ask') return;
    const esc = after.find((e) => e.sha === r.payload.escalation) ?? log.find((e) => e.sha === r.payload.escalation);
    if (esc && esc.kind === 'escalation' && esc.payload.concerns.split(' ').includes('cycle')) from = i + 1;
  });
  return after.slice(from).filter((r) => r.kind === 'acceptance').length;
}
...
export async function settle(cwd, verdict, st) {
  ...
  if (!bound && acceptanceRounds(st.records, st.log) >= BOUNDS.acceptanceRounds && !(await doneRule(st)).holds && !st.closed) bound = { kind: 'acceptanceRounds', actionClass: 'accept', target: st.slug };
...
// '.sudus/mechanisms' file -- lib/mechanisms.mjs (plan 05, already committed) stores one JSON
// entry per mechanism at '.sudus/mechanisms/<name>.json' (readMechanisms reads the directory);
// readFile(join(cwd, '.sudus/mechanisms')) on that real layout throws EISDIR, and no single write
// ever targets that literal path. The mechanisms branch below checks any path matching
// '.sudus/mechanisms/*.json' and parses it as one mechanism entry {schema, definition, review}
// instead.
//
...
// see whether the write is a genuine Sudus violation -- that needs the scratch-state walk in
// guardKernelWrite below.
async function bookkeepingViolation(cwd, path, bytes) {
  const text = bytes.toString('utf8');
  if (isMechanismDefinition(path)) {
    try {
...
// specified bookkeeping alone would create a new Sudus violation of equal or higher precedence, it
...
// before `mine` that goes null -> non-null) over-refuses. Two real cases: (1) `sudus declare`
...
// problem via 'scope', and still is, just also visible through 'record' now. (2) `sudus decide`:
...
//
// Fix round 2, finding 2 (documented trade-off, no code change): the guard below refuses only
// when the first-unmet position moves strictly earlier ("afterIdx < beforeIdx"), not merely at or
// before the action's own precedence as the spec's "equal or higher precedence" words alone would
// suggest. While an earlier predicate is already unmet -- as 'scope' is in direction (1) above --
// a write that newly breaks a DIFFERENT predicate between that one and the action's own index
// (here, 'record') is allowed: the overall first-unmet position does not move, so the guard is
// effectively off for everything between the already-open predicate and the action's own index.
// Catching that too needs the very per-predicate diff this comment already shows over-refuses
// direction (1); no one-line change closes the gap without reopening it. Recorded as a known
// limit, not a regression: the preflight-then-declare flow this guard must keep working depends
// on it.
async function firstUnmetIndex(st, cap) {
  for (const p of predicates) {
    if (p.name === 'waiting') continue;             // a state, not a violation (see guardKernelWrite)
...
export async function guardKernelWrite(cwd, path, bytes, { action }) {
  ...
  if (!refuseAs && mineIdx >= 0) {
    ...
    slug = before.slug;
    // The first unmet violation up to this action's own precedence, before and after the write;
    // the write is refused when a higher-precedence violation appears. Waiting is a state, not
    // a violation, so it is skipped: an open cycle escalation used to outrank and hide a record
    // violation the write would create, and the same refused declare went through once the
    // escalation existed. A scope breach is a violation and does outrank record, on purpose:
    // the preflight records the breach first and the declaration legalizes only later changes.
    ...
    if (afterIdx < beforeIdx) {
      ...
      because = result.reason ?? null;
    }
  }
  if (!refuseAs) return;
  // Revised 2026-09-22 (spec section 5, "Waiting and liveness"): a refusal writes no escalation
  // of its own. It counts as one administrative occurrence of this action class and target, so
  // a loop that keeps walking into the same wall reaches the ordinary cycle bounds; a single
  // refusal costs the developer nothing.
  const b = await bump(cwd, action, path);
  let note = '';
  if (b.bound) {
    if (slug === null) slug = (await readState(cwd)).slug;
    if (!openCycleEscalation(await readLog(cwd))) await writeCycleEscalation(cwd, slug, { kind: b.bound, actionClass: action, target: path, violation: refuseAs, because, refused: b.sameTarget });
    note = '; cycle escalation written';
  }
  throw new LivenessError(`sudus: writing ${path} for ${action} would create a ${refuseAs} violation${because ? ` (${because})` : ''}; resolve that first, then run the command again${note}`);
}
