...
export async function guardKernelWrite(cwd, path, bytes, { action }) {
  ...
  throw new LivenessError(`cairn: writing ${path} for ${action} would create a ${refuseAs} violation${because ? ` (${because})` : ''}; resolve that first, then run the command again${note}`);
}
