...
export function parseConcern(token) {
  ...
  const [, kind, ref, n] = m;
  if (kind === 'decision' && !/^[0-9A-HJKMNP-TV-Z]{26}$/.test(ref)) throw new DraftError(`cairn: concern token ${token} needs a decision id`);
  if (kind !== 'contract' && kind !== 'decision' && !SHA.test(ref)) throw new DraftError(`cairn: concern token ${token} needs a log SHA`);
...
export async function answer(cwd, slug, kind, opts = {}) {
  ...
  if (typeof opts.quote !== 'string' || opts.quote.trim() === '') throw new DraftError("cairn: answer needs --quote <the developer's words>: quote what the developer said in the conversation, such as their ok");
...
export const cliAnswer = asCli(async (cwd, argv, opts) => {
  ...
  return `cairn: answer ${slug} ${await answer(cwd, slug, kind, { ...opts, quote: named.quote, escalation: named.escalation })}\n`;
});
