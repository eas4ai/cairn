...
import { loadSettings, loadSettingsFull } from './settings.mjs';
...
// Fix round 2: reads through lib/settings.mjs's own exported loadSettingsFull (moved there from
// lib/evaluate.mjs so lib/adr.mjs's decide() and lib/scope.mjs's preflight() -- neither of which
// can safely take a static dependency on lib/evaluate.mjs -- could use the identical, single
// implementation too); this file already statically imports loadSettings from lib/settings.mjs, so
// the round 1 dynamic-import workaround is no longer needed. `opts.loadSettingsFull` still lets a
// test override it, the same way `opts.evaluate` already can.
export async function escalateWithRoute(cwd, draft, opts = {}) {
  const d = validateDraft(draft);
  const { settings } = await (opts.loadSettingsFull ?? loadSettingsFull)(cwd);
  const log = await readLog(cwd);
  checkConcerns(d, log, openRange(log, d.commitment));
