...
export function parseConcern(token) {
  ...
  const m = /^(finding|item|breach|transaction|contract|decision|retire):(.+?)(?:#([1-9][0-9]*))?$/.exec(token);
...
const RECORD_KIND = { item: 'item', breach: 'scope-breach', transaction: 'command-intent', retire: 'item' };
...
function rangeFor(log, d) {
  if (!d.concerns.every((t) => t.startsWith('retire:'))) return openRange(log, d.commitment);
  const r = range(log);
