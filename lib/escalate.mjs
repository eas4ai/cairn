...
// current policy digest, and route mode could never actually run through `cairn escalate`. Reads
// settings through lib/evaluate.mjs's exported loadSettingsFull instead, which resolves that
// exact calibration lookup the same way evaluate() itself does. Reached by dynamic import (the
// same way `evaluate` below already is) so this file never gains a static dependency on
// lib/evaluate.mjs; `opts.loadSettingsFull` lets a test override it the same way `opts.evaluate`
// already can.
export async function escalateWithRoute(cwd, draft, opts = {}) {
  const d = validateDraft(draft);
  const loadSettingsFull = opts.loadSettingsFull ?? (await import('./evaluate.mjs')).loadSettingsFull;
