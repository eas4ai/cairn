...
// own superseded evaluate()-based version (see the Ruling 1 comment on escalateWithRoute, above)
// used before design decisions 55/56 replaced it.
export async function decideConsequential(cwd, draft) {
  ...
  if (m.payload.outcome !== 'composite') {
    throw new DraftError(`cairn: measurement ${m.sha} routes to the developer (${m.payload.outcome}); cairn escalate --consequential instead of deciding`);
  }
...
// Ruling 1 (plan 15, task 3, controller ruling): this function used to dynamic-import lib/
// evaluate.mjs's old evaluate() and branch on whatever route it returned (developer, agent or
// capture) -- the router design decisions 55 and 56 superseded (spec section 10 replaces it with
// a composite measurement the agent itself reads before deciding, not a second decision-maker
// that can route around it). lib/evaluate.mjs is rewritten from scratch on this plan and no
// longer exports evaluate() at all, so the dynamic import and the agent/capture branches are
// removed outright here too, with no stub and no dead export: a Consequential draft always
// escalates to the developer with no evaluation, the only path this function took before when
// typesafeai was disabled. Plan 16 wires measure() into this path once it exists (this plan's
// tasks 4-10).
export async function escalateWithRoute(cwd, draft) {
  const d = validateDraft(draft);
  const log = await readLog(cwd);
  checkConcerns(d, log, openRange(log, d.commitment));
  return { route: 'developer', sha: await escalate(cwd, d) };
}
