...
function egressClass(path, settings) {
  ...
  if (path.startsWith('.sudus/output/')) return 'output';
  return null;
}
...
export async function writeMeasureBrief(cwd, r, commitment) {
  ...
  const briefPath = join(cwd, '.sudus/output', `measure-${r.slug}-${r.intentSha.slice(0, 7)}.md`);
  await mkdir(join(cwd, '.sudus/output'), { recursive: true });
