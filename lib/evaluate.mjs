...
// 10's own settings field, and Task 1 already requires its five entries to sum to 1, so the sum
// below is already a weighted mean over the full [0,1] range with nothing left to renormalize.
// Ignores `confidences` entirely by signature -- only the levels feed the composite; confidence
// only gates computeSuggested, below.
