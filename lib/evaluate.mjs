...
// Section 10: "Before any call, code checks a floor that needs no model judgment: a draft whose
// recommended option would change an Agreed requirement's text or its falsifier, would change the
// working agreement, or touches data that cannot be regenerated routes to the developer without a
// call." Reserved/protected-path writes, the fourth-attempt rule and scope rulings are already
// enforced by sections 2 and 5 independent of this floor, so floorReasons still reports them
// alongside the three named conditions (contract, agreement, data) rather than dropping them --
// narrowing the floor to the three named conditions does not relax those other checks.
//
