...
import { loadSettings, hasPassingCalibration, SETTINGS_PATH } from './settings.mjs';
...
// checks both at once. hasPassingCalibration (lib/settings.mjs) is the one place that still reads
// the calibration record kind; this function composes it rather than re-querying the log itself.
export async function assertRouteMode(cwd) {
  ...
  const pass = await hasPassingCalibration(cwd, raw);
  if (!pass) throw new RouteModeError(`cairn: route mode requires a passing calibration for policy ${policyDigest(raw)}`);
}
