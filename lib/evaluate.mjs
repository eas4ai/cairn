...
function unresolvedFindings(log) {
  ...
  for (const r of log) {
    ...
    // Sudus 4.0.0: a decline, with its reason, closes a finding as a resolution does.
    for (const f of (r.payload.findings || [])) if (!log.some((x) => (x.kind === 'resolution' || x.kind === 'decline') && x.payload.source === r.sha && x.payload.finding === f.n)) count++;
  }
  return count;
}
