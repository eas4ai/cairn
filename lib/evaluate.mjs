...
export async function measureState(cwd, D, n, C, f) {
  ...
  // recommended option's own slot). The in-tree 24-scenario benchmark showed that without the
  // losing options' text, the reach, contract and surface Score dimensions cannot separate
  // agent-expected from developer-expected drafts (16/24 overall, near-zero separation); with it,
  // 21/24 and real separation on all three
...
export async function finalizeMeasurement(cwd, slug, intentSha, { call = null, draftDigestValue, source, model = null,
  ...
  // returned, for the same reason -- the task dispatch's own Global Constraints require `cairn
...
export async function measure(cwd, draft, { transport = post, session = null, harness, env } = {}) {
  ...
  // can be detected (no --harness, no CAIRN_HARNESS, no recognized harness env var). Section 10
...
// egress-checked, closed state measureState built, not a workspace snapshot -- unlike cairn
...
// its own mkdir/writeFile for cairn brief -- moved out of lib/cli.mjs's measureCommand, which used
...
export async function writeMeasureBrief(cwd, r, commitment) {
  ...
  const any = (v) => v ?? 'any';
  const block = [
    `cairn: measure ${commitment} ${r.intentSha} pending review`,
    `brief: ${briefPath}`,
    `harness: ${launch.name}`,
...
// --- Plan 16 Task 6: completeReviewMeasurement -- cairn measure <slug> --file <path> -----------
...
// choice was already fixed at `cairn measure --brief` time and is read back off the intent, never
...
export async function completeReviewMeasurement(cwd, slug, body, { env } = {}) {
  const log = await readLog(cwd);
  const r = range(log);
  ...
  if (!intent) {
    if (reviewIntents.length) throw new MeasurementError(`cairn: measure: the review measurement for ${slug} has already completed`);
    throw new MeasurementError(`cairn: no pending review measurement for ${slug}`);
  }
  ...
  if (!isPlainObject(body)) throw new MeasurementError('cairn: measure: the answers file is not a JSON object');
