...
// bytes attached (its message names only the class and the path). Reads never follow symlinks:
// writeTreeFromPaths (lib/gitx.mjs) lstats every path and records a symlink as its own 120000
// blob (the link target text), never resolving through it to whatever it points at.
