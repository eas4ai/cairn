...
//
// A measurement is current when it is a `measurement` record in the open commitment range whose
// `draft_digest` equals `draftDigest(D)`, its named intent exists, and nothing besides that
// intent's own call and measurement records has been appended to the log since the intent --
// another decision, an answer, a receipt, a fix, or another intent all mean the repository moved
// on since the draft was measured, and the measurement is stale. The latest matching, non-stale
// measurement wins when more than one exists (a re-run after a stale one).
...
// Deviation from the brief's Step 3 snippet: the snippet's reference implementation throws the
// same message text ('no measurement for this exact draft; run cairn measure first') both when no
// measurement exists in range at all and when one exists but for a different draft digest --
// collapsing two of this task's own named three conditions (missing, stale, digest -- the task
// dispatch's own Global Constraints: "MeasurementError messages name which of the three conditions
// failed ... in words the agent can act on") into one message. Split into three distinct branches
// instead, each naming its own condition: no measurement anywhere in the open commitment range
// (missing, the brief's own verbatim message), a measurement exists in range but none matches this
// draft's digest (digest, a new message this deviation adds), and a digest match exists but the
// log moved on since its intent (stale, the brief's own verbatim message).
export async function currentMeasurement(cwd, D) {
  const log = await readLog(cwd);
  const r = range(log);
  const scope = r.start ? r.records : log;
  const measurements = scope.filter((x) => x.kind === 'measurement');
  if (!measurements.length) throw new MeasurementError('cairn: no measurement for this exact draft; run cairn measure first');
  const dd = draftDigest(D);
  const candidates = measurements.filter((x) => x.payload.draft_digest === dd);
  if (!candidates.length) throw new MeasurementError('cairn: the recorded measurement is for a different draft digest; run cairn measure again');
