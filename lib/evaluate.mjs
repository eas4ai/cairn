...
// Fix round 2 (plan 11 review): loadSettingsFull moved to lib/settings.mjs (re-exported here so
// every existing caller of `lib/evaluate.mjs`'s own export keeps working) -- lib/adr.mjs's
// decide() and lib/scope.mjs's preflight() both needed the identical calibration-aware settings
// load this file already had, but neither can safely take a static dependency on this module (this
// module already statically depends on lib/adr.mjs, and lib/adr.mjs's own preflight-adjacent
// module lib/scope.mjs, so the reverse edge would close a cycle); lib/settings.mjs depends on
// neither, and reaches this module's policyDigest only by a lazy, function-body dynamic import, so
// hosting the one shared implementation there closes no cycle anywhere. `node --input-type=module
// -e "import('./lib/scope.mjs')"` and the same for lib/adr.mjs both still load cleanly.
import { loadSettings, loadSettingsFull, SETTINGS_PATH } from './settings.mjs';
import { readFile } from 'node:fs/promises';
import { join } from 'node:path';

export { loadSettingsFull };
