...
// each pass `unit()` -- the task's own Robustness paragraph separately names "a probabilities
// object with a missing or extra key" as a defect the parser must catch on its own, distinct
// from an out-of-tolerance sum, so this checks the exact key set ("0".."4") rather than only the
// value count. The same paragraph also names "an extra question" as a defect; the brief's
...
export function parseScoreAnswers(request, bodyText) {
  ...
  for (const id of wanted) {
    ...
    if (!a || typeof a !== 'object') return { invalid: `answer ${id} missing` };
    if (a.type !== 'score') return { invalid: `answer ${id} not type score` };
    ...
    if (keys.length !== 5 || !PROB_KEYS.every((k) => k in a.probabilities)) return { invalid: `answer ${id} probabilities key set is not 0..4` };
    const vals = PROB_KEYS.map((k) => a.probabilities[k]);
    if (vals.some((v) => !unit(v))) return { invalid: `answer ${id} bad probabilities` };
    const sum = vals.reduce((s, v) => s + v, 0);
    if (Math.abs(sum - 1) > POLICY.PROB_TOLERANCE) return { invalid: `answer ${id} probabilities sum to ${sum}, not 1` };
