...
// Plan 15 Task 1: POLICY and policyDigest moved back here from lib/settings.mjs. They lived there
// (fix round 3, plan 11 review) only so loadSettings could gate route mode with them; plan 15
// removes route mode entirely (decision 55), so lib/settings.mjs no longer needs them and the
// cycle that motivated hosting them there is gone. This is a plain relocation, unchanged in shape
// -- still keyed to the seven old THRESHOLD_KEYS -- because this task only changes the settings
// shape and its direct consumers; plan 15 Task 3 rewrites this file (and policyDigest with it) for
// the composite Score design section 10 now describes.
...
export function policyDigest(settings) {
  ...
  // Plan 15 Task 1: THRESHOLD_KEYS names the seven fields typesafeai no longer has (decision 55);
  // t[k] is undefined for every one of them now, and canonicalize() refuses undefined (not valid
  // JSON). Mapped to null rather than dropped, so this digest's shape (still seven named keys)
  // does not silently change out from under any caller comparing two digests -- it just means "not
  // present under this settings shape" instead of crashing. Task 3 rewrites this whole function's
  // policy-covered fields for the composite Score design; this is only enough to keep it callable.
  return sha256(canonicalize({
    ...
    thresholds: Object.fromEntries(THRESHOLD_KEYS.map((k) => [k, t[k] ?? null])),
