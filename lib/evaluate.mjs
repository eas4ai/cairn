...
// Deviation from the plan text, discovered integrating Task 9: lib/settings.mjs's validateSettings
// (already committed) refuses `typesafeai.mode: 'route'` outright unless the caller passes a
// {calibration: {pass: true}} option through loadSettings -- every ordinary `loadSettings(cwd)`
// call in this file (kernelFacts, recoverEvaluation, evaluate) would otherwise throw a
// SettingsError for any project that has switched to route mode at all, calibrated or not, which
// contradicts this task's own "route mode routes to the developer without editing settings"
// requirement. loadSettingsFull reads the raw file first, computes the policy digest from it (no
// validation needed for that -- policyDigest only reads specific fields) and looks up whether a
// passing calibration exists for that exact policy before calling the real loadSettings with the
// answer, so a route-mode project loads (successfully or not) exactly as evaluate()'s own
// uncalibrated/otherwise logic expects. Only used internally; a project not in route mode is
// unaffected (loadSettings(cwd) with no options, same as before).
// Exported (fix round 1, finding 1): lib/escalate.mjs's escalateWithRoute -- the only path from
// `cairn escalate` to the evaluator -- needs this exact calibration-aware settings load too, for
// the identical reason (lib/settings.mjs's validateSettings refuses to load a route-mode
// project's settings without a matching {calibration} option). escalateWithRoute reaches this by
// dynamic import, the same way it already reaches `evaluate`, so lib/escalate.mjs never gains a
// static dependency on this file.
export async function loadSettingsFull(cwd) {
  let raw;
  try { raw = JSON.parse(await readFile(join(cwd, SETTINGS_PATH), 'utf8')); } catch { return loadSettings(cwd); }
  if (!raw || typeof raw !== 'object' || !raw.typesafeai || raw.typesafeai.mode !== 'route') return loadSettings(cwd);
  const policy = policyDigest(raw);
  const log = await readLog(cwd);
  const pass = log.some((r) => r.kind === 'calibration' && r.payload.policy_digest === policy && r.payload.result === 'pass');
  return loadSettings(cwd, { calibration: { pass } });
}
...
export async function kernelFacts(cwd, D) {
  const { settings } = await loadSettingsFull(cwd);
  const log = await readLog(cwd);
  const r = range(log);
...
// Deviation from the plan text, discovered integrating Task 10: lib/snapshots.mjs's
// writeWorkspaceSnapshot(cwd) (already committed, used throughout the kernel) resolves
// network_exclude by calling loadSettings(cwd) internally when no `exclude` option is given -- the
// same plain, non-route-aware call loadSettingsFull works around elsewhere in this file, but here
// it happens inside a different module entirely, so loadSettingsFull cannot intercept it.
// writeWorkspaceSnapshot already accepts `{exclude}` to skip that internal load; passing the
// already-validated settings' own network_exclude through explicitly avoids the second, unguarded
// loadSettings call without needing to touch lib/snapshots.mjs (used far too widely to safely edit
// within this plan's scope).
async function captureIdentity(cwd, D, settingsDigest, settings) {
  const ws = await writeWorkspaceSnapshot(cwd, { exclude: settings.network_exclude });
...
// Attempts one call. Three outcomes: `{crashed:true, error}` for an unknown-class throw (evaluate()
...
// a classified TransportError or an invalid answer (a 'failure' or 'response' call record is
// appended either way); `{sha, answers}` for a valid response.
...
export async function evaluate(cwd, draft, { transport = post } = {}) {
  ...
  const { settings, digest: settingsDigest } = await loadSettingsFull(cwd);
  const t = settings.typesafeai;
  ...
  const id = await captureIdentity(cwd, D, settingsDigest, settings);
...
// --- Task 9: calibration: exact bound, route-mode refusal ---------------------------------------
...
// Reads the settings file directly rather than through loadSettings/validateSettings: lib/settings.mjs
// already refuses to even load a route-mode project without a passing calibration (a project-wide
// gate, not specific to this check), so going through it here would surface that generic
// SettingsError instead of this function's own, more specific message -- and would make it
// impossible to distinguish "no calibration" from "alias model" at all, since validateSettings
// checks both at once.
export async function assertRouteMode(cwd) {
  const raw = JSON.parse(await readFile(join(cwd, SETTINGS_PATH), 'utf8'));
  const t = raw.typesafeai;
  ...
  const log = await readLog(cwd);
  const pass = log.some((r) => r.kind === 'calibration' && r.payload.policy_digest === policy && r.payload.result === 'pass');
  if (!pass) throw new RouteModeError(`cairn: route mode requires a passing calibration for policy ${policy}`);
}
