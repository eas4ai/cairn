...
// execve per entry (44 of them on one developer machine), for every git call Cairn makes; a wake
...
async function resolveTreeEntries(cwd, paths, exclude) {
  ...
  for (const p of [...new Set(paths)].sort()) {
    ...
    else if (st.isFile()) entries.push({ mode: st.mode & 0o111 ? '100755' : '100644', path: p });
