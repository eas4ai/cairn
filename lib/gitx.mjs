...
export function git(args, { cwd, input, env, expect = [0] } = {}) {
  return new Promise((resolvePromise, reject) => {
    ...
    // Review of 3.8.2: `git replace` (and the older grafts file) swapped a record for another
    // commit under its unchanged SHA, and every read followed it. Git reads objects as written.
    const child = spawn(gitBin(), args, { cwd, env: { ...(env ?? process.env), LC_ALL: 'C', GIT_NO_REPLACE_OBJECTS: '1', GIT_GRAFT_FILE: '/dev/null' }, stdio: ['pipe', 'pipe', 'pipe'] });
    const out = [], err = [];
...
// The commit a directory stands for when it is a submodule or other nested repository, as git
// records it in a tree (mode 160000): its checked-out HEAD when it has its own .git, else the
// gitlink the index records. null for a plain directory. Issue #19.
export async function gitlinkSha(cwd, p) {
  if (existsSync(join(cwd, p, '.git'))) {
    const r = await git(['rev-parse', '--verify', '-q', 'HEAD'], { cwd: join(cwd, p), expect: [0, 1, 128] });
    if (r.code === 0) return r.stdout.trim();
  }
  const staged = (await git(['ls-files', '-s', '-z', '--', p], { cwd })).stdout.split('\0').find((l) => l.startsWith('160000 '));
  return staged ? staged.split(' ')[1] : null;
}
...
async function resolveTreeEntries(cwd, paths, exclude) {
  ...
  for (const p of [...new Set(paths)].sort()) {
    ...
    if (st.isSymbolicLink()) entries.push({ mode: '120000', path: p, bytes: Buffer.from(await readlink(join(cwd, p))) });
      ...
      if (!sha) throw new GitError(`${p} is a directory; name the files under it`);
      entries.push({ mode: '160000', path: p, sha });
    }
...
async function hashEntries(cwd, entries, { write = false } = {}) {
  const w = write ? ['-w'] : [];
  // --stdin-paths reads one path per line and unquotes a leading double quote, so a path with a
  // newline in it or starting with a quote is hashed on its own below (review of 3.5.0).
  const plain = entries.filter((e) => e.mode !== '120000' && e.mode !== '160000' && !e.path.startsWith('"') && !e.path.includes('\n'));
  if (plain.length) {
