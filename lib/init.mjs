...
import { authenticateDeveloper, verifyEvidence, cliSigner, cliNonce, describeEvidence, requireQuote } from './auth.mjs';
...
export async function init(cwd, { remote, localOnly = false, signingKeyPem = null, attested = false, adopt = null, quote, sign, nonce, env } = {}) {
  ...
  // Review of 3.8.2: with the former refs still here (a migrate that stopped part way), init made
  // a second, unrelated snapshots root and migrate then refused both.
  if (L === SUDUS) for (const ref of [CAIRN.log, CAIRN.snapshots]) if (await readRef(cwd, ref)) throw new InitError(`sudus: this clone still holds ${ref}; run sudus migrate`);
  ...
  if (!hadSettings) {
    ...
    // .sudus/settings.json.
    // Review of 3.8.2: attested mode needs the quote; asking for it only after the write left a
    // settings file with no init record.
    if (!signingKeyGiven) requireQuote('init', quote);
