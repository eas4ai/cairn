...
// it, either because it is not really a Cairn log or because it was corrupted. .find() then returns
// undefined and reading rec.payload throws a raw TypeError instead of a cairn: refusal. Both call
// sites below now go through this helper so a missing init record refuses cleanly and names the
// repair, rather than crashing.
function findInitRecord(log) {
  const rec = log.find((r) => r.kind === 'init');
  if (!rec) throw new InitError('cairn: refs/cairn/log exists but has no init record; the ref is not a Cairn log or was corrupted; restore or delete refs/cairn/log before running cairn init');
  return rec;
}
