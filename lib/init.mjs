...
function findInitRecord(log) {
  const first = log[0];
  if (!first || first.kind !== 'init') throw new InitError('sudus: refs/sudus/log exists but has no init record; the ref is not a Sudus log or was corrupted; restore or delete refs/sudus/log before running sudus init');
  return first;
}
...
async function ensureSnapshotRoot(cwd) {
  if (!(await readRef(cwd, 'refs/sudus/snapshots'))) await writeWorkspaceSnapshot(cwd);
}
...
//
// Creating settings (no .sudus/settings.json yet) needs exactly one of remote/localOnly and exactly
...
export async function init(cwd, { remote, localOnly = false, signingKeyPem = null, attested = false, adopt = null, quote, sign, nonce, env } = {}) {
  ...
  const settingsPath = join(cwd, '.sudus/settings.json');
  const hadSettings = existsSync(settingsPath);
  const logHead = await readRef(cwd, 'refs/sudus/log');
  if (logHead && !hadSettings) {
    const rec = findInitRecord(await readLog(cwd));
    throw new InitError(`sudus: refs/sudus/log exists but .sudus/settings.json is missing; restore the file whose digest is ${rec.payload.settings_digest} (git checkout -- .sudus/settings.json) or write a new one and ask the developer and record the answer with sudus authorize --quote <words>`);
  }
  ...
  if (!hadSettings) {
    ...
    if (signingKeyGiven === attested) throw new InitError('sudus: init needs --signing-key <path> or --attested');
    ...
    if (reasons.length) throw new InitError(`sudus: refusing to write .sudus/settings.json: ${reasons.join('; ')}`);
    mkdirSync(join(cwd, '.sudus'), { recursive: true });
