...
// nothing itself -- the agent asks the questions in conversation first (docs/spec/cairn-v2.md). The
...
//
// Creating settings (no .cairn/settings.json yet) needs exactly one of remote/localOnly and exactly
// one of signingKeyPem/attested. Existing settings with no refs yet are adopted only with an
...
export async function init(cwd, { remote, localOnly = false, signingKeyPem = null, attested = false, adopt = null, quote, sign, nonce, env } = {}) {
  ...
  if (logHead && !hadSettings) {
    ...
    throw new InitError(`cairn: refs/cairn/log exists but .cairn/settings.json is missing; restore the file whose digest is ${rec.payload.settings_digest} (git checkout -- .cairn/settings.json) or write a new one and ask the developer and record the answer with cairn authorize --quote <words>`);
  }
  const names = await remotes(cwd);
  if (!hadSettings) {
    const remoteGiven = remote !== undefined;
    if (remoteGiven === localOnly) throw new InitError('cairn: init needs --remote <name> or --local-only');
    const authorityRemote = remoteGiven ? remote : null;
    if (authorityRemote !== null && !names.includes(authorityRemote)) throw new InitError(`cairn: authority_remote ${authorityRemote} is not a configured remote`);
    const signingKeyGiven = signingKeyPem !== null;
    if (signingKeyGiven === attested) throw new InitError('cairn: init needs --signing-key <path> or --attested');
  ...
  if (logHead) {
    ...
    throw new InitError(`cairn: settings digest ${digest} differs from the init record's ${rec.payload.settings_digest}; a settings change is a new authorization: ask the developer and record the answer with cairn authorize --quote <words>`);
  }
  if (hadSettings && adopt !== digest) {
    const creationFlagGiven = remote !== undefined || localOnly || signingKeyPem !== null || attested;
    if (creationFlagGiven) throw new InitError('cairn: settings exist; --remote, --local-only, --signing-key and --attested apply only when creating them');
    throw new InitError(`cairn: init needs --adopt <digest> to adopt existing settings at ${digest}`);
  }
...
// `init --remote <name>|--local-only --signing-key <path>|--attested [--adopt <digest>] --quote
// <words>`. --signing-key names a PEM file's path, read here (init() itself takes the key's own
