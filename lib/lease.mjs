...
import { assertInside, classify } from './paths.mjs';
...
async function checkTouch(cwd, touch) {
  ...
  for (const p of touch) {
    try { await assertInside(cwd, p); } catch (e) { throw new LeaseError(`sudus: --touch ${p}: ${e.message}`); }
...
// Issue #30: end writes a touch into the target's one mechanism (lib/mechanisms.mjs applyTouch),
// so a target that no mechanism, or several, declares left the lease covering the path only while
// it lived; end said "not written" and the next command recorded the path as a breach. Refused
// here, before any lease exists, with the same mechanism lookup end uses.
async function checkTouchTarget(cwd, target, touch) {
  const all = await readMechanisms(cwd);
  const names = Object.keys(all).filter((n) => all[n].definition.requirements.includes(target)).sort();
  if (names.length === 1) return;
  const paths = touch.join(', ');
  if (names.length === 0) throw new LeaseError(`sudus: --touch ${paths}: no mechanism declares ${target}, so end could not write it; lease implement <REQ> with the requirement the path serves, or declare the path first`);
  throw new LeaseError(`sudus: --touch ${paths}: ${names.length} mechanisms declare ${target}: ${names.join(', ')}; name one with sudus declare`);
}
...
export async function begin(cwd, { action, target, touch = [], env = process.env }) {
  ...
  if (touch.length) await checkTouchTarget(cwd, target, touch);
...
async function entryNow(cwd, p) {
  ...
  // Review of 3.8.2: --stdin-paths reads through a symlink, so a link is hashed by its target
  // text, as snapshots record it (lib/gitx.mjs hashEntries), never by what it points at.
  for (const r of rows) if (r.mode === '120000') r.sha = (await git(['hash-object', '--stdin'], { cwd, input: await readlink(join(cwd, r.rel)) })).stdout.trim();
  const plain = rows.filter((r) => !r.sha && !r.rel.startsWith('"') && !r.rel.includes('\n'));
  if (plain.length) {
