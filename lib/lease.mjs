...
async function checkTouch(cwd, touch) {
  ...
  for (const p of touch) {
    ...
    if (ignored.code === 0) throw new LeaseError(`cairn: --touch ${p} is gitignored and cannot be a mechanism input; unignore it or drop --touch`);
  }
}
...
export async function begin(cwd, { action, target, touch = [], env = process.env }) {
  ...
  if (existing) throw new LeaseError(`cairn: action lease held: ${existing.action} ${existing.target}; run cairn end when it is finished, or cairn end --abandon`);
  ...
  catch (e) { if (e instanceof CasError) throw new LeaseError('cairn: action lease held by a concurrent begin; run cairn end when it is finished, or cairn end --abandon'); throw e; }
  return sha;
}
...
// hash-object` cannot hash a directory, and `cairn end` used to report the touch not written.
...
async function entryNow(cwd, p) {
  ...
  const plain = rows.filter((r) => !r.rel.startsWith('"'));
  if (plain.length) {
...
// explicitly abandoned", but nothing before this fix ever recorded which one happened -- `cairn
// end` and `cairn end --abandon` were identical. The lease's own body (the fields begin() writes
...
// only an in-memory flag. The lease stays local and ephemeral exactly as before (refs/cairn/in-
...
// re-read refs/cairn/in-progress and act on whatever lease was live, with no check that it was the
...
// (`expect`, e.g. `cairn end --lease <sha>`, the sha `cairn begin` already prints), end() refuses
// unless the live lease is still that exact one. A caller with no `expect` (bare `cairn end` /
// `cairn end --abandon`, reconcile's own wording in the working agreement) asserts no identity and
...
export async function end(cwd, { abandon = false, expect = null } = {}) {
  ...
  if (expect && expect !== sha) {
    ...
    throw new LeaseError(`cairn: action lease ${sha} is now ${live.action} ${live.target} (session ${live.session ?? 'none'}), not the lease ${expect} this end expected; run cairn end --lease ${sha}, or cairn end --abandon`);
  }
  const lease = await readLease(cwd);
  const { tree } = await catCommit(cwd, sha);
  const body = canonicalize({ ...lease, abandoned: abandon });
  const subject = `cairn: lease ${abandon ? 'abandoned' : 'ended'} ${lease.action} ${lease.target}`;
  const terminalSha = await commitTree(cwd, { tree, parents: [sha], subject, body, trailers: [] });
  try { await deleteRefCAS(cwd, LEASE_REF, sha); }
  catch (e) { if (e instanceof CasError) throw new LeaseError('cairn: action lease changed under cairn end; run cairn wake and follow it'); throw e; }
...
export function runEnd(argv, io) {
  return refusing(io, async () => {
    ...
    if (i >= 0 && !expect) throw new LeaseError('cairn: --lease needs a value');
    await end(io.cwd, { abandon, expect });
    io.stdout(abandon ? 'cairn: lease abandoned' : 'cairn: lease ended');
  });
}
