...
async function entryNow(cwd, p) {
  ...
  const visit = async (rel) => {
    ...
    const sha = (await git(['hash-object', '--', rel], { cwd })).stdout.trim();
    rows.push(`${mode} ${sha} ${rel}`);
  };
  await visit(p);
...
export async function end(cwd, { abandon = false, expect = null } = {}) {
  ...
  if (expect && expect !== sha) {
    ...
    throw new LeaseError(`cairn: action lease ${sha} is now ${live.action} ${live.target} (session ${live.session ?? 'none'}), not the lease ${expect} this end expected; run cairn reconcile`);
  }
