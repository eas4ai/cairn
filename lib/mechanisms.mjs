...
import { recordManagedWrite, isDeclared } from './scope.mjs';
...
// What decides what a check detects (spec revision 10): its command, working directory and
// results mode. Inputs, documents, the execution identity and the requirement list decide only
// when a receipt is current, so changing them leaves a mechanism review bound.
export const detectionDigest = (def) => sha256(canonicalize({ command: def.command, cwd: def.cwd, results: def.results }));

// Whether a requirement's review metadata binds the entry's definition. A review written before
// revision 10 carries only the full definition digest and binds that exact definition.
export function reviewHolds(rv, entry, REQ) {
  if (!rv || !entry.definition.requirements.includes(REQ)) return false;
  return rv.detectionDigest !== undefined ? rv.detectionDigest === detectionDigest(entry.definition) : rv.definitionDigest === entry.definitionDigest;
}

// The reviews a redeclare keeps: each that still holds under the new definition. A review from
// before revision 10 that bound the old definition first gains the old detection digest, so an
// unchanged command keeps it too.
export function carriedReviews(existing, definition) {
  if (!existing) return {};
  const next = { definition, definitionDigest: definitionDigest(definition) };
  const kept = {};
  for (const [REQ, rv] of Object.entries(existing.review)) {
    const known = rv.detectionDigest === undefined && rv.definitionDigest === existing.definitionDigest ? { ...rv, detectionDigest: detectionDigest(existing.definition) } : rv;
    if (reviewHolds(known, next, REQ)) kept[REQ] = known;
  }
  return kept;
}
...
async function writeEntry(cwd, name, entry) {
  const dir = join(cwd, layoutOf(cwd).mechanisms);
  await mkdir(dir, { recursive: true });
  const tmp = join(dir, `.${name}.json.tmp`);
  const bytes = canonicalize(entry);
  await guardKernelWrite(cwd, `${layoutOf(cwd).mechanisms}/${name}.json`, bytes, { action: 'declare' });
  await writeFile(tmp, bytes);
  await rename(tmp, join(dir, `${name}.json`));
  await recordManagedWrite(cwd, `${layoutOf(cwd).mechanisms}/${name}.json`, bytes);
}
...
export async function readMechanisms(cwd) {
  let files;
  try { files = await readdir(join(cwd, layoutOf(cwd).mechanisms)); } catch (e) { if (e.code === 'ENOENT') return {}; throw e; }
  const out = {};
  for (const f of files.filter((f) => f.endsWith('.json') && !f.startsWith('.')).sort()) {
    const name = basename(f, '.json');
    let obj;
    try { obj = parseStrict(await readFile(join(cwd, layoutOf(cwd).mechanisms, f), 'utf8')); } catch (e) { fail(`${layoutOf(cwd).mechanisms}/${f} is not canonical JSON: ${e.message}`); }
    ...
    if (!NAME.test(name) || obj.schema !== 1 || !obj.definition || !obj.review || Object.keys(obj).length !== 3 || !validDefinitionShape(obj.definition)) {
      fail(`${layoutOf(cwd).mechanisms}/${f} is not a valid mechanism entry (kernel-managed path breach)`);
    }
...
export async function declare(cwd, name, rawDefinition) {
  ...
  const review = existing && existing.definitionDigest === digest ? existing.review : carriedReviews(existing, definition);
  await writeEntry(cwd, name, { schema: 1, definition, review });
  return { definitionDigest: digest };
}

// Review of 3.8.2: this picked domain files by a `^Prefix:` sniff and its own sort, so a byte
// order mark before the header hid a file's blocks from check while readSpec, which the frozen
// set and lint read, still had them. The block is the one readSpec resolves.
export async function requirementDigest(cwd, REQ) {
  const block = (await readSpec(cwd)).blocks.get(REQ);
  if (!block) fail(`${REQ} is not in docs/spec`);
  return { textDigest: block.textDigest, agreed: block.status?.kind === 'Agreed' };
}

import { catCommit, git } from './gitx.mjs';
import { decodeRecord, readLog } from './records.mjs';

export function reviewBinds(entry, REQ, textDigest) {
  const m = entry.review[REQ];
  return reviewHolds(m, entry, REQ) && m.textDigest === textDigest;
}

// Issue #31: the detection digest of the definition a receipt ran under. A receipt names only its
// definition digest, so the definition is found where the kernel wrote it: the entry itself, a
// review accepted against it, or the mechanism file's committed history. A history version counts
// only when its own digest equals the receipt's. Null when none holds it.
export async function receiptDetection(cwd, name, entry, digest) {
  if (digest === entry.definitionDigest) return detectionDigest(entry.definition);
  const known = Object.values(entry.review ?? {}).find((rv) => rv.definitionDigest === digest && rv.detectionDigest);
  if (known) return known.detectionDigest;
  const file = `${layoutOf(cwd).mechanisms}/${name}.json`;
  const commits = await git(['log', '--format=%H', '--', file], { cwd, expect: [0, 128] });
  if (commits.code !== 0) return null;
  for (const c of commits.stdout.split('\n').filter(Boolean)) {
    const shown = await git(['show', `${c}:${file}`], { cwd, expect: [0, 128] });
    if (shown.code !== 0) continue;
    let def;
    try { def = parseStrict(shown.stdout).definition; } catch { continue; }
    if (def && definitionDigest(def) === digest) return detectionDigest(def);
  }
  return null;
}

// Issue #31: the latest fail receipt for REQ that review mechanism accepts: a ran receipt of a
// mechanism in `names` that declares REQ, recording fail for REQ at `textDigest`, whose definition
// has that mechanism's current detection digest. `any` says whether REQ has a fail receipt at all.
export async function usableFailReceipt(cwd, log, mechanisms, REQ, textDigest, names = Object.keys(mechanisms)) {
  const fails = log.filter((r) => r.kind === 'receipt' && r.payload.status === 'ran' && r.payload.results.some((x) => x.requirement === REQ && x.result === 'fail'));
  const known = new Map();
  for (const r of [...fails].reverse()) {
    const name = r.payload.mechanism, entry = mechanisms[name];
    if (!names.includes(name) || !entry || !entry.definition.requirements.includes(REQ)) continue;
    if (r.payload.results.find((x) => x.requirement === REQ).text_digest !== textDigest) continue;
    const key = `${name} ${r.payload.definition_digest}`;
    if (!known.has(key)) known.set(key, await receiptDetection(cwd, name, entry, r.payload.definition_digest));
    if (known.get(key) === detectionDigest(entry.definition)) return { sha: r.sha, any: true };
  }
  return { sha: null, any: fails.length > 0 };
}
...
export async function reviewMechanism(cwd, name, REQ, failReceiptSha) {
  ...
  if (!log.some((r) => r.sha === failReceiptSha)) fail(`${failReceiptSha} is not a record on ${layoutOf(cwd).log}`);
  ...
  // Issue #31: the review binds the detection digest (spec revision 10), so the fail receipt needs
  // the current command, working directory and results mode, not the exact definition: a redeclare
  // that only added a requirement used to make every older fail receipt unusable.
  const det = await receiptDetection(cwd, name, entry, p.definition_digest);
  if (det !== detectionDigest(entry.definition)) fail(`receipt ${failReceiptSha} ran under ${det === null ? `definition ${p.definition_digest}, which neither ${name}'s review metadata nor its committed history holds` : `another command, working directory or results mode than ${name} declares now`}; make ${REQ}'s violating example fail, run sudus check ${REQ}, then review mechanism ${REQ} with that receipt`);
  ...
  const review = { ...entry.review, [REQ]: { definitionDigest: entry.definitionDigest, detectionDigest: detectionDigest(entry.definition), textDigest, failReceipt: failReceiptSha } };
...
//
// Issue #14: a changed path that a declared input already covers (the input `src` covers
// `src/new.rs`) changes nothing the mechanism reads. Declaring it again only changed the
// definition digest, which made every receipt of the mechanism stale and left a file to commit,
// so it is reported as `covered: [{path, by}]` and the definition is written only for new paths.
export async function applyTouch(cwd, lease, outcome) {
  if (!lease || !Array.isArray(lease.touch) || lease.touch.length === 0) return { added: [], dropped: [], unclaimed: [], covered: [] };
  const dropped = [...outcome.unchanged];
  const changed = [...outcome.changed];
  if (changed.length === 0) return { added: [], dropped, unclaimed: [], covered: [] };
  ...
  if (names.length !== 1) {
    ...
    return { added: [], dropped, unclaimed: changed.map((path) => ({ path, reason })), covered: [] };
  }
  const name = names[0];
  const inputs = all[name].definition.inputs;
  const covered = changed.flatMap((path) => { const by = inputs.find((d) => isDeclared(path, [d])); return by ? [{ path, by }] : []; });
  const added = changed.filter((path) => !isDeclared(path, inputs));
  if (added.length) await declare(cwd, name, { ...all[name].definition, inputs: [...inputs, ...added] });
  return { added, dropped, unclaimed: [], covered };
}
