...
// bookkeeping would create a Cairn violation of equal or higher precedence than declare's is
...
// shape, so a hand-edited or corrupted `.cairn/mechanisms/*.json` (a kernel-managed path: only
...
export async function readMechanisms(cwd) {
  ...
  for (const f of files.filter((f) => f.endsWith('.json') && !f.startsWith('.')).sort()) {
    ...
    // Fix round 2 finding 2 (Minor): this message used to start with the literal word "cairn:"
    // itself; lib/cli.mjs's main() always prepends its own "cairn: " to whatever a command throws,
    // so a caller that let this MechanismError reach main() unwrapped printed a doubled "cairn:
    // cairn: ..." line. Every other refusal in this module (and the parallel fix in lib/cli.mjs's
...
// hook on lib/lease.mjs's own end() -- but bin/cairn.mjs's actual command wiring (lib/cli.mjs)
// never imports lib/mechanisms.mjs at all, only lib/lease.mjs's runEnd, so that side effect never
// ran in the shipped binary and `cairn end` never wrote a touched path into any definition. The
...
// prints one `cairn: touch <path> not written: <reason>` line per unclaimed path and lets the
