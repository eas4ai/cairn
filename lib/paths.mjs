...
import { LAYOUTS, isOutputPath } from './layout.mjs';

export class PathError extends Error { constructor(m) { super(m); this.name = 'PathError'; } }
// Both layouts' state directories are reserved in every project, so a path under the former
// layout's directory is never a plain path even after `sudus migrate` moved the project.
export const RESERVED = [...LAYOUTS.map((l) => `${l.dir}/**`), 'docs/spec/**', 'docs/decisions.jsonl', 'AGENTS.md'];
export const PROTECTED = [...LAYOUTS.map((l) => l.settings), 'docs/spec/**', 'AGENTS.md'];
...
export const KERNEL_MANAGED = [...LAYOUTS.flatMap((l) => [l.mechanisms, `${l.mechanisms}/**`]), 'docs/decisions.jsonl'];
export const OUTPUT = '.sudus/output/**';
...
export function validatePath(p) {
  ...
  // A leading colon is a git pathspec magic signature (':(exclude)...', ':!...'); refusing it here
  // means every caller that hands an input straight to `git ls-files` as a pathspec (writeInputSnapshot)
  // is already safe against it. Carried from plan 01's review.
  if (p.startsWith(':')) throw new PathError(`path begins with : (pathspec magic) in ${p}`);
...
// The name a path is stored under. On a case-insensitive filesystem (macOS by default) a case or
// Unicode-normalization variant of a path opens the stored file while globs and string
// comparisons see another name; each segment becomes the directory entry it opens. A segment
// that opens nothing is kept as written, and the path reads as absent.
export async function storedPath(cwd, p) {
  const fold = (s) => s.normalize('NFC').toLowerCase();
  const out = [];
  for (const seg of p.split('/')) {
    const dir = join(cwd, ...out);
    let names;
    try { names = await readdir(dir); } catch { return p; }
    if (names.includes(seg)) { out.push(seg); continue; }
    const same = names.filter((n) => fold(n) === fold(seg));
    if (same.length !== 1) return p;
    try { await lstat(join(dir, seg)); } catch { return p; }
    out.push(same[0]);
  }
  return out.join('/');
}
