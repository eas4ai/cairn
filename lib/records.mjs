...
export const SCHEMAS = {
  ...
  // carries the actual launch instruction cairn brief chose. report() reads them from here, not
  ...
  'measurement': { intent: ref, call: nullable(ref), draft_digest: digest, source: oneOf('jev', 'review'), model: nullable(str), levels: list(obj({ dimension: DIMENSION, level: json, confidence: json })), composite: nullable(json), veto: nullable(oneOf('reach', 'contract', 'surface')), suggested: nullable(oneOf('agent', 'developer')), outcome: oneOf('floor', 'unavailable', 'veto', 'composite', 'indeterminate'), reason: str },
