...
const refPair = obj({ 'refs/sudus/log': nullable(sha), 'refs/sudus/snapshots': nullable(sha) });
...
export function encodeRecord(kind, target, payload) {
  ...
  return { subject: `sudus: ${kind} ${target}`, body, trailers: [['Sudus-Schema', SCHEMA], ['Sudus-Digest', sha256(body)]] };
}
...
export function verifyEnvelope(commit) {
  ...
  if (tr.length !== 2 || tr[0][0] !== 'Sudus-Schema' || tr[1][0] !== 'Sudus-Digest') throw new RecordError('expected exactly the two trailers Sudus-Schema and Sudus-Digest');
  if (tr[0][1] !== SCHEMA) throw new RecordError(`unreadable schema ${tr[0][1]}`);
  const bytes = commit.bodyBytes ?? Buffer.from(commit.body, 'utf8');
  if (tr[1][1] !== sha256(bytes)) throw new RecordError('body digest does not match Sudus-Digest');
  try { return parseStrict(bytes); } catch (e) { throw new RecordError(`body: ${e.message}`); }
}
export function decodeRecord(commit) {
  const m = /^sudus: (\S+) (\S+)$/.exec(commit.subject);
