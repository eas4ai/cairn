...
import { currentReceipt } from './check.mjs';
...
// What already ran, so the adversary never runs it again: per requirement, the receipt current at
// the workspace, the one wake decides with, and the fail receipt its mechanism review binds. Issue
// #33: the newest receipt was named instead, so a violating example checked after the fix and then
// restored read as a current failure; a newer receipt that is not current is now marked so.
async function receiptLines(cwd, log, t, mechanisms) {
  const L = [];
  const outcome = (r, req) => (r.payload.status === 'ran' ? r.payload.results.find((y) => y.requirement === req).result : 'error');
  for (const req of t.requirements) {
    const cur = await currentReceipt(cwd, log, req);
    const newest = log.findLast((x) => x.kind === 'receipt' && x.payload.results.some((y) => y.requirement === req));
    const curRec = cur && log.find((x) => x.sha === cur.sha);
    if (curRec) L.push(`${req}: current receipt ${curRec.sha} from mechanism ${curRec.payload.mechanism}: ${outcome(curRec, req)}`);
    else L.push(newest ? `${req}: no current receipt` : `${req}: no receipt`);
    if (newest && newest !== curRec) L.push(`${req}: newest receipt ${newest.sha} from mechanism ${newest.payload.mechanism}: ${outcome(newest, req)}, not current`);
...
export async function brief(cwd, slug, opts = {}) {
  ...
  const text = renderBrief({
    ...
    blocks: await specBlocks(cwd, entries, t.requirements), mechanisms, receipts: await receiptLines(cwd, log, t, mechanisms),
