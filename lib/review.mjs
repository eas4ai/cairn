...
export function exclusionClass(p, settings) {
  ...
  if (p.startsWith('.sudus/output/')) return 'output';
...
export function renderBrief({ slug, roadmap, blocks, mechanisms, rev, obligations, changed = [], manifest, launch }) {
  ...
  L.push('', '## Changed paths (start snapshot to reviewed snapshot; A added, M modified, D deleted)', ...changed.map((d) => `${d.status} ${d.path}`));
...
export async function brief(cwd, slug, opts = {}) {
  ...
  const text = renderBrief({
    ...
    rev: rev.payload, obligations: await interfaceObligations(cwd, settings, t.start.payload.snapshot, rev.payload.snapshot),
    changed: await diffTree(cwd, (await readSnapshot(cwd, t.start.payload.snapshot, 'workspace')).tree, snap.tree), manifest: proj.manifest, launch,
  });
  ...
  const briefPath = path.join(cwd, '.sudus/output', `brief-${payloadDigest.slice(7)}.md`);
...
export async function report(cwd, slug, body, opts = {}) {
  ...
  if (nowTree !== then.tree) throw new ReviewError('sudus: report: the workspace differs from the reviewed snapshot');
...
export async function resolve(cwd, slug, n, explanation, opts = {}) {
  ...
  if (open.length === 0) throw new ReviewError(`sudus: resolve: no unresolved finding ${n}${opts.source ? ` on ${opts.source}` : ''}`);
...
export async function accept(cwd, slug, body, opts = {}) {
  ...
  if (submitted.length === 0 && gateTree === reportedAt.tree) throw new ReviewError('sudus: accept: nothing submitted since the report and no delta to examine');
