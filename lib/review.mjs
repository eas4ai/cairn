...
export async function accept(cwd, slug, body, opts = {}) {
  ...
  // genuinely names a new workspace snapshot, unlike report()'s.
  const currentTree = await currentWorkspaceTree(cwd, settings), reportedAt = await readSnapshot(cwd, rep.payload.snapshot, 'workspace');
  if (submitted.length === 0 && currentTree === reportedAt.tree) throw new ReviewError('cairn: accept: nothing submitted since the report and no delta to examine');
  ...
  const findings = checkFindings(body.findings ?? []);
  // Every refusal has now run; only here does accept() write the workspace snapshot its own
  // record names, and it records exactly the sha that write produced.
  const snapshot = await writeWorkspaceSnapshot(cwd);
  const delta = await diffTree(cwd, reportedAt.tree, currentTree);
