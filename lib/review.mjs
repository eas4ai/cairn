...
export async function accept(cwd, slug, body, opts = {}) {
  ...
  for (const f of ledger(after, slug)) {
    // Only the acceptance that rejects a finding the second time escalates it; a later acceptance
    // that judges other resolutions wrote a second escalation for the same finding.
    if (f.status === 'rejected' && f.rejections === 2 && rejected.some((x) => x.resolution === f.resolutions.at(-1)?.sha)) {
      ...
      const reasons = f.resolutions.flatMap((res) => {
        const a = after.find((x) => x.kind === 'acceptance' && x.payload.rejected.some((v) => v.resolution === res.sha));
        return a ? [`${a.payload.rejected.find((v) => v.resolution === res.sha).reason} (acceptance ${a.sha})`] : [];
      });
      await escalate(cwd, {
        ...
        because: `The adversary rejected it twice: ${reasons.join('; ') || 'see the acceptance records'}.`,
