...
export async function report(cwd, slug, body, opts = {}) {
  ...
  if (typeof body.model !== 'string' || body.model === '' || body.model === 'any') throw new ReviewError('cairn: report: model is the model the adversary actually ran as; the brief\'s any accepts any name but is not one');
  if (body.transport !== 'local' && body.transport !== 'remote') throw new ReviewError('cairn: report: transport is the one the adversary actually ran over, local or remote; the brief\'s any accepts either but is not one');
