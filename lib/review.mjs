...
export function renderBrief({ slug, roadmap, blocks, mechanisms, rev, obligations, changed = [], carriedFrom = null, manifest, launch, pairs = [], projectionDigest = null }) {
  ...
  L.push('## Report',
    ...
    ...pairs.map(([q, x]) => `  ${q} ${x}`),
    `- "interface_attempts": one {"path", "text"} for each path below${obligations.length ? '' : ' (none: write [])'}`,
    ...obligations.map((p) => `  ${p}`),
...
export async function brief(cwd, slug, opts = {}) {
  ...
  const text = renderBrief({
    ...
    pairs: requiredPairs(t), projectionDigest: proj.projectionDigest,
  });
