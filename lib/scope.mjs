...
// Fix round 1 item 8: one `git hash-object --stdin-paths` spawn for the whole candidate list
// instead of one spawn per file; git already reads a symlink's own target bytes (not the resolved
// file) when given its path this way, so entryOf no longer needs to do that by hand. lstat is
// still needed per path for the mode bit; a path that vanished between listing and lstat is
// treated as absent (falls through to the deleted-paths loop below), matching
// lib/gitx.mjs's writeTreeFromPaths.
export async function workspaceDelta(cwd, treeSha) {
  ...
  for (const p of await workspacePaths(cwd)) {
    ...
    present.push(p);
    modes.set(p, st.isSymbolicLink() ? '120000' : (st.mode & 0o111) ? '100755' : '100644');
  }
  const shas = present.length ? (await git(['hash-object', '--stdin-paths'], { cwd, input: present.map((p) => p + '\n').join('') })).stdout.trim().split('\n') : [];
  const presentSet = new Set(present);
  const out = [];
  present.forEach((p, i) => {
    const e = entries.get(p);
    const cur = { mode: modes.get(p), sha: shas[i] };
...
// canonical and schema-valid. One line per write: "<path> <sha256 of the bytes>".
...
async function managedWriteDigest(cwd, path) {
  let text;
...
export async function preflight(cwd, log, { command = null } = {}) {
  ...
  // Fix round 1 item 4: an unreadable mechanisms directory (one corrupted entry breaks
  // readMechanisms for all of them) is itself the breach for the corrupted path; every other
  // path -- declared or not -- is left unrecorded this pass rather than guessed at with no
  // reliable declaration set.
  let mechanisms, mechanismsOk = true;
  try { mechanisms = await readMechanisms(cwd); } catch { mechanisms = {}; mechanismsOk = false; }
