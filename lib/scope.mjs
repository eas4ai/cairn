...
export async function preflight(cwd, log, { command = null } = {}) {
  ...
  // Fix round 3 (plan 11 review): loadSettings (lib/settings.mjs) is itself calibration-aware now,
  // so a route-mode project with a genuinely passing calibration at the exact current policy digest
  // is not refused here -- one layer before every STATE_CHANGING command's own handler (including
  // lib/escalate.mjs's escalateWithRoute) is ever reached.
  let settings;
