...
// in the ADR (lib/adr.mjs BOOKKEEPING_KINDS). Wake's review, accept and done predicates and the
// report, accept and reviewState checks compare through this (spec item 50: valid bookkeeping
// may not generate its own violation). Before, the developer's ok on an escalation moved the
// workspace off the last acceptance, and the adversary ran one more round to examine the
// answered line alone.
