...
async function workspacePaths(cwd) {
  ...
  return new Set([...tracked, ...untracked].filter((p) => !p.startsWith('.sudus/output/')));
}
...
export async function kernelManagedValid(cwd, path, baseTree) {
  ...
  return path.startsWith('.sudus/mechanisms/') && path.endsWith('.json');
}
...
function authorized(path, digests, log) {
  const key = path === 'AGENTS.md' ? 'agreement' : path === '.sudus/settings.json' ? 'settings' : 'spec';
