...
import { loadSettingsFull } from './settings.mjs';
import { protectedDigests, specDigest, agreementDigest } from './auth.mjs';
...
export async function workspaceDelta(cwd, treeSha) {
  ...
  for (const p of await workspacePaths(cwd)) {
    ...
    if (st.isSymbolicLink()) { modes.set(p, '120000'); symlinks.push(p); }
      ...
      modes.set(p, (st.mode & 0o111) ? '100755' : '100644');
  ...
  for (const p of entries.keys()) if (!presentSet.has(p)) out.push({ path: p, change: 'deleted' });
...
export async function preflight(cwd, log, { command = null } = {}) {
  ...
  // Fix round 2 (plan 11 review): reads through lib/settings.mjs's loadSettingsFull instead of
  // loadSettings directly, so a route-mode project with a genuinely passing calibration at the
  // exact current policy digest is not refused here -- one layer before every STATE_CHANGING
  // command's own handler (including lib/escalate.mjs's escalateWithRoute) is ever reached.
  let settings;
  try { ({ settings } = await loadSettingsFull(cwd)); }
  ...
  // Fix round 2 (plan 11 review): lib/auth.mjs's protectedDigests(cwd) (already committed) computes
  // its own `settings` digest via loadSettings(cwd), plain, with no {calibration} option -- the
  // identical circularity this fix round is closing everywhere else, one file over. lib/auth.mjs is
  // outside this fix round's authorized files, so its two other, pure, settings-independent pieces
  // (specDigest, agreementDigest) are called directly and the settings digest is computed here from
  // the already-loaded, calibration-aware `settings` above -- byte-identical to what
  // protectedDigests itself would compute (sha256(canonicalize(settings)), the same formula
  // lib/settings.mjs's own loadSettings uses). The `export { protectedDigests }` re-export below is
  // untouched: tests/scope.test.mjs calls it directly and unrelated to this internal use.
  const digests = { spec: specDigest(cwd), agreement: agreementDigest(cwd), settings: sha256(canonicalize(settings)) };
  const shas = [];
  ...
  for (const { path } of await workspaceDelta(cwd, tree)) {
    ...
    // lib/snapshots.mjs's writeWorkspaceSnapshot resolves network_exclude via its own internal,
    // unguarded loadSettings(cwd) call when no `exclude` option is given -- the identical
    // circularity loadSettingsFull exists to work around, one file over; lib/snapshots.mjs is
    // outside this fix round's authorized files, so the already-loaded, calibration-aware
    // `settings` from above is passed through explicitly instead.
    try { firstObserved ??= await writeWorkspaceSnapshot(cwd, { exclude: settings.network_exclude }); }
    catch (e) {
...
export const STATE_CHANGING = new Set(['begin', 'end', 'check', 'declare', 'review-mechanism', 'review', 'brief', 'report', 'resolve', 'accept', 'escalate', 'answer', 'reply', 'dispute', 'item', 'outside', 'fix', 'decide', 'realize', 'promote', 'authorize', 'start', 'done', 'supersede', 'scope', 'calibrate']);
