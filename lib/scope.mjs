...
export async function preflight(cwd, log, { command = null } = {}) {
  ...
  for (const { path, mode, sha } of await workspaceDelta(cwd, tree)) {
    ...
    // tests and declarations before cairn start, and every one of them read as an undeclared
...
export async function dispose(cwd, breachRef, disposition) {
  ...
  if (!/^[0-9a-f]{40}$/.test(breachRef)) {
    ...
    if (open.length === 0) throw new ScopeError(`cairn: no open scope-breach for ${breachRef}`);
    if (open.length > 1) throw new ScopeError(`cairn: ${open.length} open scope-breaches for ${breachRef}: ${open.map((b) => b.sha).join(', ')}; name one by sha`);
...
export async function cmdScope(argv, { cwd, stdout }) {
  ...
  if (!breach || !disposition) throw new ScopeError('cairn: usage: cairn scope <breach-sha or path> keep|restore');
