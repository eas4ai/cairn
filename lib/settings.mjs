...
export function validateSettings(s, { mechanisms = [], calibration = null, remotes = null } = {}) {
  ...
  // One-way: a documents path is refused only when it is nested at or under a source root, not
  // merely when the two patterns' languages overlap (a source root nested under a documents
  // directory, e.g. documents: ['docs'] with source: ['docs/lib/**'], is fine).
  const underSourceRoot = (path, glob) => { const root = stem(glob); if (root === '') return false; const p = stem(path); return p === root || p.startsWith(root + '/'); };
  for (const d of documents) for (const g of source) if (underSourceRoot(d, g)) out.push(`documents ${d} lies below source ${g}`);
