...
// Fix round 2 (plan 11 review): validateSettings above refuses to load a `mode: 'route'` project's
// settings at all unless the caller threads a {calibration: {pass: true}} option through -- an
// invariant that turned out not to be honored by every loadSettings caller kernel-wide.
// loadSettingsFull is the one, single, exported place that resolves it: it reads the raw settings
// file, and for a route-mode project only, looks up whether a passing calibration record exists at
// the project's exact current policy digest before calling loadSettings with the answer. Every
// caller that used to call loadSettings(cwd) plain (lib/adr.mjs's decide(), lib/scope.mjs's
// preflight(), lib/escalate.mjs's escalateWithRoute, and lib/evaluate.mjs's own internal reads) now
// calls this instead, so there is exactly one copy of this logic to maintain.
//
// policyDigest is lib/evaluate.mjs's own function (POLICY, questions, envelope rule text, router
// version, thresholds, caps, egress -- an evaluator concept, not a settings one), and
// lib/evaluate.mjs already statically imports from lib/adr.mjs; lib/adr.mjs and lib/scope.mjs both
// already statically import loadSettings from this file. A static import here of anything from
// lib/evaluate.mjs would close a cycle (settings.mjs -> evaluate.mjs -> adr.mjs -> settings.mjs, and
// similarly through scope.mjs). The dynamic import below is evaluated lazily, only when a project
// is actually in route mode (the rare path), so it never participates in the static module graph at
// all -- verified with `node --input-type=module -e "import('./lib/scope.mjs')"` and
// `import('./lib/adr.mjs')` after this change, both of which still load cleanly.
export async function loadSettingsFull(cwd) {
  ...
  const { readLog } = await import('./records.mjs');
  const { policyDigest } = await import('./evaluate.mjs');
