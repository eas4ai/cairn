...
import { validateGlob, RESERVED, PathError } from './paths.mjs';
...
const TOP = ['schema', 'authority_remote', 'outside', 'source', 'interfaces', 'data', 'network_exclude', 'signing_key', 'attribution', 'developer', 'harness', 'typesafeai'];
// Keys a settings file may leave out; every other key in TOP is required. adversary_rules (issue
// #13) is the host's limits for the adversary, which the brief prints under their own heading.
const OPTIONAL = ['adversary_rules'];
const HARNESS = ['adversary_model', 'adversary_transport'];
// Plan 15: the seven live/shadow route thresholds and the mode switch are gone (decision 55 --
// section 10's composite is always advisory, never a route authority). weights/agent_ceiling/
// confidence_floors replace them (decision 56: weights is a real field again, not a removed one).
...
// Review of 3.8.2: that fixed list of prefixes plus the underscore-only long-token alternative
// missed three common secret shapes, all still refused here rather than added to the prefix list,
// since none of them starts with a fixed prefix: an AWS secret access key (mixed case, digits, and
// a "/", e.g. wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY); a base64 blob using "+", "/" or "=", not
// just underscore; and a JWT (three dot-separated base64url segments, each long enough that a
// dotted version string such as jev-1.13.0 can never match all three). The last alternative below
// generalizes the long-token check to that wider charset, but only when the value also mixes an
// upper-case letter, a lower-case letter and a digit -- a plain lower-case, dotted or hyphenated
// model id (claude-opus-5-5, jev-1.13.0, gpt-5.1-codex) never has an upper-case letter, so it still
// validates clean. It also needs one run of 20 or more characters with no hyphen or dot: a long
// mixed-case model id (Llama-4-Maverick-17B-128E-Instruct-FP8) is short words, a secret is
// dense.
const SECRET_VALUE = /^(-----BEGIN|(sk|pk|tsk|ghp|gho|xox[abps])[-_][A-Za-z0-9_-]{10,}$|AKIA[0-9A-Z]{16}$|(?=[A-Za-z0-9_]{32,}$)(?=[A-Za-z0-9_]*[A-Za-z])(?=[A-Za-z0-9_]*[0-9])[A-Za-z0-9_]{32,}$|[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{10,}$|(?=[A-Za-z0-9+/=_.-]{32,}$)(?=[A-Za-z0-9+/=_.-]*[A-Z])(?=[A-Za-z0-9+/=_.-]*[a-z])(?=[A-Za-z0-9+/=_.-]*[0-9])(?=[A-Za-z0-9+/=_.-]*[A-Za-z0-9+/=_]{20})[A-Za-z0-9+/=_.-]{32,}$)/;
...
function closed(obj, allowed, path, out, optional = []) {
  ...
  for (const k of Object.keys(obj)) {
    ...
    else if (!allowed.includes(k) && !optional.includes(k)) out.push(`unknown field ${path}${k}`);
  }
}
// A dimension map (typesafeai.weights or typesafeai.confidence_floors) is a closed object over
// exactly the five score dimensions, each a number in [0,1]. Reports every kind of shape defect
// at once (missing dimension, extra key, out-of-range value) rather than stopping at the first.
...
// Section 10's composite (lib/evaluate.mjs computeComposite) is a weighted mean over [0,1] with
// no renormalization -- that is only a weighted mean when the five weights already sum to 1.
// dimensionMap alone (shape and per-entry [0,1] range) never checked this; only run the sum
// check once every entry is a valid number, so a missing/out-of-range entry gets exactly the one
// dimensionMap message for it rather than also a confusing sum mismatch computed from a bad
// value (e.g. a string coerced by `+`, or a missing key's undefined poisoning the sum to NaN).
function weightsSum(v, out) {
  if (!isObj(v) || DIMENSIONS.some((k) => typeof v[k] !== 'number' || v[k] < 0 || v[k] > 1)) return;
  const sum = DIMENSIONS.reduce((s, k) => s + v[k], 0);
  if (Math.abs(sum - 1) > 1e-6) out.push(`typesafeai.weights must sum to 1, sums to ${sum}`);
}
...
export function validateSettings(s, { mechanisms = [], remotes = null } = {}) {
  const out = [];
  ...
  closed(s, TOP, '', out, OPTIONAL);
  const { signing_key, ...rest } = s; secrets(rest, '', out);
  if ('adversary_rules' in s && !(Array.isArray(s.adversary_rules) && s.adversary_rules.every((r) => typeof r === 'string' && r.trim() !== '' && !r.includes('\n')))) out.push('adversary_rules must be a list of one-line rules');
...
export const SETTINGS_PATH = '.sudus/settings.json';

// Plan 15: POLICY and policyDigest moved out of this file entirely (they existed here only to let
// loadSettings gate route mode, which is gone -- decision 55, section 10's composite is always
// advisory, never a live/shadow authority switch). lib/evaluate.mjs (plan 15 Task 3) owns them now.
// loadSettings goes back to a plain read-parse-validate-digest function, exactly as every other
// kind of record loader in this kernel: no calibration option, no route-mode gate.
export async function loadSettings(cwd, opts = {}) {
  let text;
  const path = layoutOf(cwd).settings;
  try { text = await readFile(join(cwd, path), 'utf8'); }
  catch (e) {
    if (e.code === 'ENOENT') throw new SettingsError([`no ${path}; run sudus init`]);
    throw new SettingsError([`${path} is not readable: ${e.code}`]);
  }
  let settings;
  try { settings = JSON.parse(text); } catch (e) { throw new SettingsError([`${path} is not valid JSON: ${e.message}`]); }
