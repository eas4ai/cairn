...
export const ALWAYS_EXCLUDED = ['.git', ...LAYOUTS.map((l) => l.output)];
...
export async function listPaths(cwd, pathspec = []) {
  ...
  // .claude/worktrees/, say) as its directory with a trailing slash. It is not project content,
  // and git cannot hash it, so it is left out.
  return { tracked: await ls(['--cached']), untracked: (await ls(['--others', '--exclude-standard'])).filter((p) => !p.endsWith('/')) };
}
...
async function commitSnapshotFromTree(cwd, payload, tree) {
  ...
  const body = canonicalize(payload), head = await readRef(cwd, L.snapshots);
  const sha = await commitTree(cwd, { tree, parents: head ? [head] : [], subject: `${L.name}: snapshot ${payload.kind}`, body, trailers: [[L.schemaTrailer, SCHEMA], [L.digestTrailer, sha256(body)]] });
  await updateRefCAS(cwd, L.snapshots, sha, head);
  return sha;
}
...
async function resolveExclude(cwd, exclude) {
  ...
  try { await access(join(cwd, layoutOf(cwd).settings)); } catch { return []; }
...
// The kind a snapshot commit's subject names, workspace or input, or null for any other commit.
export function snapshotKind(subject) {
  return /^(?:sudus|cairn): snapshot (workspace|input)$/.exec(subject)?.[1] ?? null;
}
export async function readSnapshot(cwd, sha, expectedKind) {
  const c = await catCommit(cwd, sha);
  const kind = snapshotKind(c.subject);
  if (!kind) throw new KindError(`${sha} is not a snapshot commit`);
  let payload; try { payload = verifyEnvelope(c); } catch (e) { throw new SnapshotError(`${sha}: ${e.message}`); }
  const reasons = []; check(obj(SNAPSHOT_SCHEMAS[kind]), payload, 'snapshot', reasons);
