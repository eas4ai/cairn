...
// Deviation from the plan text: the plan's replacement body throws `new Error('cairn: no allowed
// workspace snapshot; run cairn start')` when no base is found, but this function's own
// already-passing test (tests/snapshots.test.mjs's "allowedBase is the newest start or scope
// snapshot, never merely the newest snapshot commit") asserts allowedBase(cwd, []) === null before
// any start record exists, matching this module's original plan-01 contract and the Shared
// interfaces line in docs/plans/overview.md ("allowedBase(cwd, log) -> sha // newest allowed
// workspace snapshot", silent on the pre-start case). preflight() itself never calls allowedBase
// before confirming a start record exists in the log, so the null path is never reached from this
// plan's own code; the null return is kept so plan 01's fixture keeps passing.
