...
export async function readSpec(cwd) {
  ...
  let entries;
  try { entries = (await readdir(dir, { withFileTypes: true })).filter((e) => e.name.endsWith('.md')); }
  catch (e) {
    // Never let a raw ENOENT (or any other readdir failure) carry the absolute host path out of
    // this module; report the repository-relative directory and nothing else.
    if (e.code === 'ENOENT') throw new SpecError(`${SPEC_DIR} is missing`);
    throw new SpecError(`${SPEC_DIR} is not readable: ${e.code}`);
  }
  const names = [], texts = {}, unreadable = [];
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    // A directory whose name happens to end in .md (or any other non-regular entry) is not a
    // domain file; reading it would throw EISDIR. Report it by name rather than crash on
    // text.split() with a null text, or on parseDomainFile(null).
    if (!entry.isFile()) { unreadable.push(entry.name); continue; }
    try { texts[entry.name] = await readFile(join(dir, entry.name), 'utf8'); names.push(entry.name); }
    catch { unreadable.push(entry.name); }
  }
  ...
  return { texts, unreadable, map: parseSpecMap(texts['overview.md'] ?? ''), domains, roadmap: parseRoadmap(texts['roadmap.md'] ?? ''), blocks };
}
export async function lint(cwd) {
  const { texts, unreadable, map, domains, roadmap, blocks } = await readSpec(cwd);
  const out = [], f = (file, line, reason) => out.push({ file: `${SPEC_DIR}/${file}`, line, reason });
  for (const name of unreadable) f(name, 1, `${name} is not a readable file`);
  ...
  // Check every identifier-shaped token on every line, including a block's own opening line (a
  // reference elsewhere on that line was previously never checked). A block's own declared id is
  // always in blocks (it is what declared it), so scanning the whole line never flags it. Reporting
  // by "not a defined identifier" rather than "known domain prefix" also catches a mistyped prefix
  // such as LOP-001, which no real domain ever defines.
  for (const [file, text] of Object.entries(texts)) text.split('\n').forEach((line, i) => {
    for (const m of line.matchAll(REF_RE)) if (!blocks.has(m[1])) f(file, i + 1, `reference to absent identifier ${m[1]}`);
  });
