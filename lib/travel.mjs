...
export async function missingRefsLine(cwd) {
  ...
  // Review of 3.8.2: a migrate stopped between its log and snapshots moves holds only the former
  // snapshots ref, and was named `sudus init`, which made a second, unrelated snapshots root.
  if (L === SUDUS) {
    const held = [];
    for (const ref of [CAIRN.log, CAIRN.snapshots]) if (await readRef(cwd, ref)) held.push(ref);
    if (held.length) return `sudus migrate  (this clone still holds ${held.join(', ')}; migrate moves it, or names the fetch when the authority remote already holds the moved refs)`;
  }
  ...
  // wake used to name it forever. Review of 3.8.2: this is checked before the local-only line,
  // which named a bare `sudus init` that refuses without --adopt.
  const uncommitted = loaded && (await git(['status', '--porcelain', '--', L.settings], { cwd })).stdout.trim();
...
export async function push(cwd, { branch } = {}) {
  ...
  for (const ref of order) {
    ...
    if (oids[ref] && !(await isAncestor(cwd, oids[ref], local))) {
      // Review of 3.8.2: a clone with records of its own was told the remote is ahead and named a
      // fetch git refuses as a non-fast-forward. The remote's objects are fetched, with --refmap=
      // so the refspecs sudus start installed move no ref, to tell a clone that is behind from one
      // that diverged.
      const fetched = await git(['fetch', '-q', '--refmap=', '--no-write-fetch-head', remote, ref], { cwd, expect: [0, 1, 128] });
      if (fetched.code === 0 && !(await isAncestor(cwd, local, oids[ref]))) {
        if (ref === branchRef) throw new TravelError(`sudus: ${ref} on ${remote} and this clone have diverged; merge or rebase onto ${remote}'s ${branch}, then push again`);
        const count = async (range) => (await git(['rev-list', '--count', range], { cwd })).stdout.trim();
        const [here, there] = [await count(`${oids[ref]}..${local}`), await count(`${local}..${oids[ref]}`)];
        const what = ref === L.log ? 'record(s)' : 'snapshot(s)';
        throw new TravelError(`sudus: ${ref} on ${remote} and this clone have diverged: ${here} ${what} here are not on ${remote} and ${there} there are not here; the ${ref === L.log ? 'log' : 'snapshot history'} is append-only and never merged. To keep ${remote}'s records, run: git fetch ${remote} +${ref}:${ref}; the ${here} here are then dropped and their work is recorded again`);
      }
