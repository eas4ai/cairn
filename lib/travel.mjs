...
import { readLog, refFieldsOf } from './records.mjs';
...
// validation belongs here, at cairn push's own post-push check, not on wake's ordinary path (see
...
async function afterPush(result, cwd) {
  ...
  if (repairs.length) throw new TravelError(`cairn: pushed, but a cross-reference is still unresolved; ${repairs[0].command}`);
  return result;
}
...
export async function validateAfterFetch(cwd) {
  ...
  // .cairn/settings.json (a raw fixture, or a repository whose settings were never written) --
  ...
  // slug with no matching start record is the ordinary spec-phase state between `cairn init` and
  // the project's first `cairn start` -- the spec-phase tail writes Current: by hand before that
  ...
  const everStarted = log.some((r) => r.kind === 'start');
  if (current && everStarted && !log.some((r) => r.kind === 'start' && r.payload.slug === current)) {
