...
export function fetchCommand(authority) {
  return `git fetch ${authority} 'refs/sudus/log:refs/sudus/log' \\\n  'refs/sudus/snapshots:refs/sudus/snapshots'`;
}
...
export async function push(cwd, { branch } = {}) {
  ...
  const order = ['refs/sudus/snapshots', 'refs/sudus/log', branchRef];
...
export async function validateAfterFetch(cwd) {
  ...
  const snapSet = await chain(cwd, 'refs/sudus/snapshots');
  const tips = { 'refs/sudus/log': await readRef(cwd, 'refs/sudus/log'), 'refs/sudus/snapshots': await readRef(cwd, 'refs/sudus/snapshots') };
  const gaps = [];
  const seen = new Set();
  ...
  for (const r of log) {
    ...
    for (const v of refValues(r.payload, snapshotFields)) {
      if (typeof v === 'string' && SHA_RE.test(v) && !snapSet.has(v)) add('refs/sudus/snapshots', v, `record ${r.sha}`);
    }
    for (const v of refValues(r.payload, recordFields)) {
      if (typeof v === 'string' && SHA_RE.test(v) && !logSet.has(v)) add('refs/sudus/log', v, `record ${r.sha}`);
    }
  }
  ...
  if (current && commitmentOpen && !log.some((r) => r.kind === 'start' && r.payload.slug === current)) {
    add('refs/sudus/log', `start ${current}`, `the start record for ${current}`);
  }
  let adr = [];
  ...
  for (const line of adr) {
    for (const k of ['answer', 'escalation', 'record']) {
      const v = line[k];
      if (typeof v === 'string' && SHA_RE.test(v) && !logSet.has(v)) add('refs/sudus/log', v, `ADR line ${line.id}`);
    }
  }
