...
import { layoutOf, resolveRef, isOutputPath, LAYOUTS } from './layout.mjs';

const OUTPUT_EXCLUDES = LAYOUTS.map((l) => `:(exclude)${l.output}`);
...
// A branch with no commits yet (git init, then sudus init, then the first authorize) has no HEAD:
// rev-parse exits 1, which is null here, not an error.
async function headSha(cwd) { const r = await git(['rev-parse', '--verify', '-q', 'HEAD'], { cwd, expect: [0, 1] }); return r.code === 0 ? r.stdout.trim() : null; }
...
async function neverTracked(cwd, paths) {
  ...
  // Issue #11: a planned directory such as .sudus/ also holds .sudus/output/, check output and
  // briefs that its own .gitignore keeps local; the kernel never plans to commit those, so they are
  // never forced in, and a start or promote no longer commits them and push no longer publishes them.
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    if ((xy === '??' || xy === '!!') && !isOutputPath(rec.slice(3))) untracked.push(rec.slice(3));
  ...
  for (let n = 0; n < s.plan.writes.length; n++) {
    ...
    if (w.store === 'file') {
      ...
      if (head !== expected) {
        ...
        // Issue #11: a project that tracked .sudus/output/ before 3.5.0 would still commit each
        // change to it; the output directories are excluded from every branch write.
        await git(['commit', '-q', '--allow-empty', '--only', '-m', w.message, '--', ...w.paths, ...OUTPUT_EXCLUDES], { cwd });
...
async function identityNow(cwd, entry, results) {
  if (entry.store.startsWith('file:')) {
    ...
    const snap = results.find((e) => resolveRef(cwd, e.store) === layoutOf(cwd).snapshots);
    const commitSha = branch?.identity ?? snap?.identity ?? null;
    return commitSha ? await contentDigestAt(cwd, commitSha, path) : null;
  }
  // Issue #5: a store recorded under the former layout's ref name resolves through the layout.
  return entry.store === 'HEAD' ? await headSha(cwd) : await readRef(cwd, resolveRef(cwd, entry.store));
}
