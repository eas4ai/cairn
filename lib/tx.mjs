...
export async function applyWrites(cwd, s, failAfterWrite = -1) {
  ...
  for (let n = 0; n < s.plan.writes.length; n++) {
    ...
    if (w.store === 'file') {
      ...
      if (cur !== expected) {
        const c = cur ? await catCommit(cwd, cur) : null;
        if (!(c && c.parents[0] === expected)) throw conflict(tx, 'refs/cairn/snapshots', cur, expected, 'a new snapshot commit');
        out = cur;
      } else {
...
function storeLabel(w) {
  if (w.store === 'file') return `file:${w.path}`;
...
// Fix round 1 finding 3: an append-only ref or HEAD naturally advances past the identity this
// transaction recorded as later, unrelated commands run -- that is progress, not drift. Ancestry,
// not equality, is the right check for those: the recorded identity must still be reachable from
// the store's current tip, or something rewrote history out from under it. A file has no ancestry
// to walk, so it is checked by exact digest.
async function driftedEntry(cwd, entry) {
  if (entry.store.startsWith('file:')) return fileDigest(cwd, entry.store.slice(5)) !== entry.identity;
  const cur = entry.store === 'HEAD' ? await headSha(cwd) : await readRef(cwd, entry.store);
  if (cur === entry.identity) return false;
  if (!cur || !entry.identity) return true;
  const r = await git(['merge-base', '--is-ancestor', entry.identity, cur], { cwd, expect: [0, 1] });
  return r.code !== 0;
}
...
export async function pendingTransaction(cwd, log) {
  ...
  for (const entry of closing?.payload.results ?? []) {
    if (await driftedEntry(cwd, entry)) return last;
  }
  return null;
}
