...
export async function preIdentities(cwd, plan) {
  ...
  return { refs: { 'refs/sudus/log': await readRef(cwd, 'refs/sudus/log'), 'refs/sudus/snapshots': await readRef(cwd, 'refs/sudus/snapshots') },
...
export async function applyWrites(cwd, s, failAfterWrite = -1) {
  ...
  for (let n = 0; n < s.plan.writes.length; n++) {
    ...
    if (w.store === 'file') {
      ...
      const cur = await readRef(cwd, 'refs/sudus/snapshots');
      const expected = s.pre.refs['refs/sudus/snapshots'];
      if (cur !== expected) {
        ...
        if (!(candidate && candidate.parent === expected)) throw conflict(tx, 'refs/sudus/snapshots', cur, expected, 'a new snapshot commit');
        out = cur;
      } else {
...
function storeLabel(w) {
  ...
  if (w.store === 'snapshot') return 'refs/sudus/snapshots';
  if (w.store === 'log') return 'refs/sudus/log';
  return w.store;
}
...
async function identityNow(cwd, entry, results) {
  if (entry.store.startsWith('file:')) {
    ...
    const snap = results.find((e) => e.store === 'refs/sudus/snapshots');
...
export async function effectsHappened(cwd, s, log, intent) {
  ...
  if ((await readRef(cwd, 'refs/sudus/snapshots')) !== s.pre.refs['refs/sudus/snapshots']) return true;
...
export async function recover(cwd, tx) {
  ...
  try {
    ...
    const restored = [
      { store: 'refs/sudus/log', identity: await readRef(cwd, 'refs/sudus/log') },
      { store: 'refs/sudus/snapshots', identity: await readRef(cwd, 'refs/sudus/snapshots') },
