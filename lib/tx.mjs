...
export async function acquireLock(cwd, tx) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (read.gone) {
        if (++goneRetries > 5) throw new TxError('cairn: cairn-tx.lock keeps disappearing and reappearing; remove it by hand and retry');
        attempt--; continue;
      }
...
function writeDigest(w) {
  ...
  throw new TxError(`cairn: unknown store ${w.store}`);
}
...
export async function stage(cwd, tx, plan, pre) {
  ...
  for (let n = 0; n < plan.writes.length; n++) {
    ...
    if (w.store === 'file') {
      ...
      if (!Array.isArray(w.paths) || w.paths.length === 0) throw new TxError('cairn: a branch write needs at least one path');
...
export async function applyWrites(cwd, s, failAfterWrite = -1) {
  ...
  for (let n = 0; n < s.plan.writes.length; n++) {
    ...
    if (w.store === 'file') {
      ...
      // depends on knowing the file's prior content: it is idempotent by checking the file already
      // ends with its own staged bytes (a resumed or recovered transaction adds nothing twice), and
      // otherwise appends to whatever is actually on disk right now, under this transaction's lock
      // -- preserving a concurrent command's own append rather than silently discarding it.
      const curBytes = existsSync(join(cwd, w.path)) ? readFileSync(join(cwd, w.path)) : Buffer.alloc(0);
      const appendBytes = s.bytes(n);
      const alreadyAppended = curBytes.length >= appendBytes.length && curBytes.subarray(curBytes.length - appendBytes.length).equals(appendBytes);
      const finalBytes = alreadyAppended ? curBytes : Buffer.concat([curBytes, appendBytes]);
      if (!alreadyAppended) writeAtomic(join(cwd, w.path), finalBytes);
      ...
      // and an idempotent replay alike, so a crash between the bytes landing and this line, followed
      // by recovery re-entering this same write, still leaves the ledger naming what is actually on
      // disk -- otherwise every start, supersede or promote that ever recovered from a crash would
      // leave docs/decisions.jsonl reading as an unrecorded (and therefore breaching) hand edit at
      // the next preflight.
      ...
      const cur = await readRef(cwd, 'refs/cairn/snapshots');
      const expected = s.pre.refs['refs/cairn/snapshots'];
      if (cur !== expected) {
        ...
        // `cairn begin` (or anything else) wrote to the same ref at the same moment. readSnapshot
        ...
        if (!(candidate && candidate.parent === expected)) throw conflict(tx, 'refs/cairn/snapshots', cur, expected, 'a new snapshot commit');
        out = cur;
      } else {
    ...
    results.push(out);
    if (n === failAfterWrite) throw new TxError(`cairn: simulated crash after write ${n} (test only)`);
  }
  return results;
}
...
function storeLabel(w) {
  ...
  if (w.store === 'snapshot') return 'refs/cairn/snapshots';
  if (w.store === 'log') return 'refs/cairn/log';
  return w.store;
}
...
async function finish(cwd, s, tx, intentSha, fn, failAfterWrite = -1) {
  ...
  const storeResults = s.plan.writes.map((w, i) => ({ store: storeLabel(w), identity: results[i] }));
...
async function identityNow(cwd, entry, results) {
  if (entry.store.startsWith('file:')) {
    ...
    const snap = results.find((e) => e.store === 'refs/cairn/snapshots');
...
async function driftRepair(cwd, tx, closing) {
  ...
  return `cairn: transaction ${tx} completed, but drifted: ${lines.join('; ')}; restore each store to its recorded identity, or accept the change and continue`;
}
...
// whether to name `cairn recover`. Only the newest transaction is checked, not the whole log's
...
export async function recover(cwd, tx) {
  ...
  try {
    ...
    if (!s) {
      ...
      const repair = `cairn: staging for transaction ${tx} is missing; its intent ${intent.sha} names ${intent.payload.writes.length} writes with digests ${intent.payload.writes.map((w) => w.digest).join(', ')}; check each store against them, restore ${intent.payload.pre.head} as HEAD if no write landed, then run the command again`;
    ...
    // intent, including refs/cairn/log's -- which can never be true again, since the log is
    ...
    const restored = [
      { store: 'refs/cairn/log', identity: await readRef(cwd, 'refs/cairn/log') },
      { store: 'refs/cairn/snapshots', identity: await readRef(cwd, 'refs/cairn/snapshots') },
