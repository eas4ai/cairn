...
import { isCurrent, attemptState } from './check.mjs';
...
define('review mechanism', (st) => {
  ...
  const hint = fr ? `; its latest fail receipt is ${fr.sha}` : '; no fail receipt exists yet, run cairn check against a violating example first';
  return unmet('review mechanism', stale.requirement, `review metadata for ${stale.requirement} is missing or bound to another definition or text digest${hint}`);
});
