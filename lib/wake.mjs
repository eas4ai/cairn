// lib/wake.mjs
import { readLog, range, fixBase } from './records.mjs';
...
define('fix', async (st) => {
  ...
  for (const it of st.log.filter(mine)) {
    ...
    // Issue #4: the base is the contract in force when the fix was recorded, not the newest start.
    const base = fixBase(st.log, st.log.findIndex((r) => r.sha === fix.sha));
    if ((await protectedChanged(st, base, fix.payload.snapshot)).length) return unmet('fix', it.payload.slug, 'the fix snapshot changes a protected path');
