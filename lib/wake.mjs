...
export async function wake(cwd, opts = {}) {
  ...
  // refs/cairn/snapshots (log fetched, snapshots left at an older, real commit) fell through to
  // the 'recover' predicate below, which reads that same staleness as transaction drift and names
  // `cairn recover <tx>` for a start that already completed cleanly -- an actively wrong diagnosis,
  ...
  // log yet (lib/travel.mjs), so wake right after cairn init still prints the pending-
  // initialization line, and both crash-fixture variants still name cairn recover <tx> (the crash
  // always happens before the first start's terminal record lands, so that exemption still
  // applies at the moment wake is called). cairn show (lib/cli.mjs's requireRefs) and cairn push's
  // own post-push check (lib/travel.mjs's push) keep their own calls too, for the case where
  // neither of those commands is preceded by a fresh cairn wake.
...
define('scope', (st) => { const b = openBreaches(st.log)[0]; return b ? unmet('scope', b.path, `scope-breach ${b.sha} observed ${b.path} undeclared at ${b.snapshot}`) : null; });
...
define('waiting', (st) => {
  ...
  if (st.settings && st.settings.developer === 'absent' && isFloorOrVetoEscalation(st.log, o.e)) return { ...base, exit: 4 };
  return base;
});
...
define('run', (st) => {
  ...
  for (const { requirement: req } of st.set) {
    ...
    const tried = attempts(st.log, req, { since: st.start.sha, siblings: st.set.map((x) => x.requirement) });
