...
export const FETCH_LINE = (remote) => `git fetch ${remote} 'refs/sudus/log:refs/sudus/log' \\\n  'refs/sudus/snapshots:refs/sudus/snapshots'`;
...
export async function readState(cwd, { now = Date.now(), session = process.env.SUDUS_SESSION ?? null } = {}) {
  ...
  st.settings = (await attempt('.sudus/settings.json', () => loadSettings(cwd)))?.settings ?? null;
  ...
  st.mechanisms = (await attempt('.sudus/mechanisms', () => readMechanisms(cwd))) ?? {};
  ...
  st.lease = await attempt('refs/sudus/in-progress', () => readLease(cwd));
