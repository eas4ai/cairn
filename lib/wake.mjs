...
// Fix round 1, item 10: attempted and reverted. Section 5's predicate text is "review metadata
// has that binding and fail receipt; no declared product input changed" -- the product clause was
// never checked, so I compared the fail receipt's recorded product_digest (lib/check.mjs's
// productDigest: the declared-input tree, documents excluded) against a fresh one computed from
// the current workspace. That literal comparison could not stay in this branch: every
// tests/helpers/loop.mjs mechanism (mechanismFor) declares a per-requirement flags/<REQ> file as
// an input with `documents: []`, so that flag is itself part of "product." The flag necessarily
// differs between the fail receipt (flag content 'fail') and any later pass (flag content
// 'pass') -- an ordinary, correct passReq(), with nothing else changed, was flagged "review
// mechanism" as if a real product change had happened, since the flag toggle is indistinguishable
// from one under this definition. Reproduced: with the check in place, 13 of this file's own
// tests failed, all of them ordinary passReq() flows with no unreviewed change at all. A correct
// fix needs a way to tell a pass/fail-controlling declared input apart from a genuine product
// input -- either a schema change recording a product digest at review time (lib/mechanisms.mjs's
// reviewMechanism, outside this round's file list) or a different, authorized definition of
// "declared product input" than "every non-document declared input." Left as a Waiting item for
// the coordinator; reviewBound keeps its original (pre-round) binding/fail-receipt check only.
...
// Fix round 1, item 3's own dependency: guards against being reached with no review at all
// (`rev` null). Under wake's own cascading precedence this never happened (the 'review' predicate
// would already be the first unmet one), but lib/cycle.mjs's guardKernelWrite now tests every
// predicate at or before its own action's precedence independently against a scratch state, so
// 'report' can be asked about a state where 'review' is also unmet; without this guard, the
// `rev.payload.snapshot` reference inside the later unmet(...) message throws on a null `rev`.
...
define('resolve', (st) => {
  ...
  if (f.rejections >= 2) {
    return { verdict: 'Resolvable', action: 'escalate', target: `${st.slug} ${f.n}`,
      ...
      predicate: 'an escalation concerns the finding' };
  }
  return unmet('resolve', `${st.slug} ${f.n}`, `finding ${f.n} on the ${f.source.kind} ${f.source.sha.slice(0, 7)} has no accepted resolution`);
});

// Fix round 1, item 11(b): guards against being reached with no report at all. Under wake's own
// cascading precedence this never happened (the 'report' predicate would already be the first
// unmet one), but lib/cycle.mjs's guardKernelWrite (item 3) tests every predicate at or before its
// own action's precedence independently against a scratch state, so 'accept' can now be asked
// about a state where 'report' would also be unmet; without this guard, `rep.payload.snapshot`
// below throws on a null `rep`.
