...
export const PREDICATES = {
  ...
  // reading of docs/spec/cairn-v2.md); these three strings dropped them. Restored verbatim.
...
export async function wake(cwd, opts = {}) {
  ...
  // Kernel fix round (plan 14 fixture, defect 1, ruling A): section 4's "After fetch, wake
  // validates all cross-references and names the exact fetch or push repair; it never guesses"
  // names an after-fetch check, not a condition on every wake() call. lib/travel.mjs's own
  // validateAfterFetch is called unconditionally on this ordinary path -- including immediately
  // after cairn init, and during crash recovery before a first cairn start's terminal record
  // lands -- so its roadmap-Current:-vs-start check always fires during a brand-new project's
  // ordinary spec-authoring window and masks every other verdict (the pending-initialization
  // 'supersession' message, and 'recover's own guidance) behind a nonsensical "cairn push" line
  // that no clone and no push can act on (reproduced in tests/fixture.test.mjs and
  // tests/fixture-crash.test.mjs; see the plan 14 report's "Kernel defects found by the fixture").
  // The spec names no other place a plain `cairn wake` call is itself the after-fetch check, so
  // this function no longer runs it at all; the real after-fetch call sites are cairn show
  // (lib/cli.mjs's requireRefs) and cairn push's own post-push check (lib/travel.mjs's push).
