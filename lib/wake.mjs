...
import { disputes, concerns, retiredBy } from './escalate.mjs';
...
define('fix', async (st) => {
  ...
  for (const it of st.log.filter(mine)) {
    ...
    if (!pass || pass.result !== 'pass' || (passAt < st.log.indexOf(fix) && !(await inputsAt(st, st.log[passAt], fix.payload.snapshot)))) return unmet('fix', it.payload.slug, `${it.payload.source} has no current pass at or after the fix`);
  }
  return null;
});
...
function openFindings(st) {
  ...
  for (const src of findingSources(st)) for (const f of src.payload.findings) {
    ...
    // an instead answer leaves it open, so wake names its next resolution, not another escalation.
    const escalations = concerns(st.log, `finding:${src.sha}#${f.n}`);
    const settled = disputes(st.log, src.sha, f.n) || escalations.some((e) => pending.has(e.sha));
    if (!resolved && !settled) out.push({ source: src, n: f.n, rejections: rejections(src.sha, f.n), escalated: escalations.length > 0 });
  }
  return out;
}
...
define('promote', (st) => {
  ...
  const item = st.log.find((r) => r.kind === 'item' && r.payload.kind === 'backlog' && !promoted.has(r.sha) && !retiredBy(st.log, r.sha));
