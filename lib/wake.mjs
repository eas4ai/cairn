...
// The latest review's findings, every report's, and a 3.x acceptance's, plus those a supersession
// carried in.
function findingSources(st) {
  const rev = latest(st, 'review');
  return [...carriedFindingSources(st.log, st.range), rev, ...st.records.filter((r) => r.kind === 'report' || r.kind === 'acceptance')].filter(Boolean);
}
