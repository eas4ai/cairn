...
const current = JSON.parse(readFileSync(join(root, "package.json"), "utf8")).version, from = parse(current);
...
const log = readFileSync(join(root, "CHANGELOG.md"), "utf8"), heading = new RegExp(`^## ${next.replace(/\./g, "\\.")} - \\d{4}-\\d{2}-\\d{2}$`, "m");
