...
The developer is never asked to run a command. When an escalation waits, state its five fields in plain prose, ask for the decision, a different direction or a question, and wait. Record the answer in the developer's own words: `cairn answer <slug> ok | instead | ask --quote "<their words>"`. Read the queue with `cairn decisions`; after the developer has read a decision with you, record it with `cairn decisions --read <id> --quote "<their words>"`. After changing `docs/spec/`, `AGENTS.md` or `.cairn/settings.json` between commitments, state what changed and what would be bound, ask, and on ok run `cairn authorize --quote "<their words>"`; a change request or a question is `cairn authorize instead | ask --quote "<their words>"`, which binds nothing. Never use a choice widget for these questions; a sentence and a wait is the prompt. After Done, open the next work with `/next-feature`.
