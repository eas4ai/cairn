...
test('helper builds a throwaway repo with a commit', async (t) => {
  ...
  assert.equal(await repo.readRef('refs/sudus/log'), null);
  assert.equal(await repo.git('config', 'user.name'), 'Sudus Test');
});
...
test('makeProject returns an initialized project with settings, an init record and an origin remote', async (t) => {
  ...
  assert.equal(await p.readRef('refs/sudus/snapshots') !== null, true);
