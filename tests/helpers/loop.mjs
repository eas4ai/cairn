...
export async function loopRepo({ reqs = ['DEMO-001'], slug = 'first', settings = {} } = {}) {
  ...
  const steps = {
    ...
    async report(findings = []) {
      ...
      return add('report', slug, { slug, snapshot: rev.payload.snapshot, brief, model: 'test-model', transport: 'local', boundary: 'enforced', builder_model: null, projection_digest: 'sha256:' + '1'.repeat(64), attempts, findings, interface_attempts: [] });
    },
    ...
    async accept({ accepted = [], rejected = [], findings = [] } = {}) {
      ...
      return add('acceptance', slug, { slug, report: rep.sha, snapshot: await snap(), delta_digest: 'sha256:' + '4'.repeat(64), accepted: accepted.map((s) => ({ resolution: s, reason: 'ok' })), rejected: rejected.map((s) => ({ resolution: s, reason: 'not fixed' })), findings });
    },
