...
test("an old report re-dated, however it is edited, does not name the new commit and names a new reviewer; a new report with the same words does not (LOOP-020)", () => {
  ...
  for (const edit of [(s) => s, (s) => s + "\n", (s) => s.replace("reviewer: a fresh reviewer", "reviewer: a fresh reviewer!")]) {
    ...
    assert.match(wake(root), /does not name commit [0-9a-f]{7} in its examined: list, so it was not written for this review/);
  }
...
test("each finding is carried by the one line citing its number and beginning with its words; malformed reports are repairs (LOOP-020)", () => {
  const root = repo();
  const carried = (entries, findings) => { review(root, entries, findings); commit(root, "carried"); return wake(root); };
  assert.match(carried([], ["empty input breaks the thing"]), /finding 1 is not carried: no review line cites it: empty input breaks the thing/);
  ...
  assert.match(carried(["resolved: a defect, unrelated walkthrough typo fixed"], ["a defect"]), /no review line cites it/, "words alone never carry");
  ...
  assert.match(carried(["resolved: the gate, fixed (independent 1)", "resolved: the gate, again (independent 1)", "resolved: the gate, the parser and the message are wrong. fixed (independent 2)"], ["the gate", "the gate, the parser and the message are wrong"]), /finding 1 is not carried: 2 review lines cite it/);
