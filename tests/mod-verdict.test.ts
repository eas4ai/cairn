...
import { bandOf, cannotStart, changesVerdict, expressionOf, MULTI_WORD_ACTIONS, parseWake, usesSudus, type Verdict } from '../mod/verdict.ts'
...
test('a Resolvable verdict reads action, target, reason, predicate and the layout line', () => {
  ...
  assert.deepEqual(bandOf(v), { label: 'Resolvable', color: 'green', subject: 'run WTE-001', lines: ['no current receipt carries a result for WTE-001'] })
})
...
test('Waiting is unsure, or mad with no developer to answer; Done is happy', () => {
  ...
  assert.deepEqual(bandOf(w), { label: 'Waiting for the developer', color: 'yellow', subject: '', lines: ['escalation abc awaits an answer'] })
...
test('Done names its commitment, and Waiting shows the escalation question', () => {
  ...
  assert.deepEqual(bandOf(d), { label: 'Done', color: 'cyan', subject: 'first', lines: ['done record closes first and no backlog item waits'] })
...
test('an exit-3 line is sick and a repair; wake that cannot be reached is sleepy', () => {
  ...
  assert.deepEqual(bandOf(line), { label: 'Repair', color: 'red', subject: '', lines: ['sudus: outside a project; run /new-project or /existing-project'] })
  const missing = parseWake('', 1)
  assert.equal(missing.kind, 'missing')
  assert.equal(expressionOf(missing, false), 'sleepy')
  assert.equal(bandOf(missing)?.label, 'Not reachable')
})
...
test('a long reason reaches the band whole: the band wraps it, nothing cuts it', () => {
  const reason = 'x'.repeat(300) + '\u{1F600}'.repeat(20)
  assert.deepEqual(bandOf(parseWake(`verdict: Resolvable\naction: fix gate\nreason: ${reason}\npredicate: p\n`, 0))?.lines, [reason])
})
...
test('review mechanism is read as one action, and every multi-word action wake names is known', () => {
  ...
  assert.equal(bandOf(v)?.subject, 'review mechanism WTE-001')
